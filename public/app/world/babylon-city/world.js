...
function lowBuilding(n,x,z,w,d,h,label){let r=new B.TransformNode(n,scene);r.position.set(x,0,z);r.parent=world;let shell=box('shell',w,h,d,0,h/2,0,mats.dark,r);shell.checkCollisions=true;box('glassFront',w*.82,h*.66,.3,0,h*.52,d/2+.18,mats.glass,r);box('goldCap',w+1,.55,d+1,0,h+.28,0,mats.gold,r);textPlane(label,0,h*.73,d/2+.38,Math.min(w*.8,24),2.7,r);districtNodes.push(r);return r}
function plaza(){
...
function makeHuman(name,x,z,scale=1,master=false){
let r=new B.TransformNode(name,scene);r.position.set(x,0,z);r.scaling.setAll(scale);r.parent=world;
let skin=master?mats.white:mats.concrete,cloth=master?mats.gold:mats.dark;
const rig={phase:0,arms:[],legs:[],head:null,body:null,bodyBaseY:3.1};
function pcyl(n,diam,h,px,py,pz,material,parent=r){let o=B.MeshBuilder.CreateCylinder(n,{diameter:diam,height:h,tessellation:low?12:20},scene);o.position.set(px,py,pz);o.material=material;o.parent=parent;o.receiveShadows=true;shadow.addShadowCaster(o);return o}
rig.body=pcyl('body',1.45,2.5,0,3.1,0,cloth);rig.bodyBaseY=3.1;
let headPivot=new B.TransformNode('headPivot',scene);headPivot.position.set(0,4.9,0);headPivot.parent=r;rig.head=headPivot;
let head=B.MeshBuilder.CreateSphere('head',{diameter:1.25,segments:low?8:12},scene);head.material=skin;head.parent=headPivot;head.receiveShadows=true;shadow.addShadowCaster(head);
for(let s of [-1,1]){
  let legPivot=new B.TransformNode(s<0?'leftLegPivot':'rightLegPivot',scene);legPivot.position.set(s*.38,2.25,0);legPivot.parent=r;rig.legs.push(legPivot);
  pcyl('leg',.42,2.3,0,-1.1,0,mats.dark,legPivot);
  let armPivot=new B.TransformNode(s<0?'leftArmPivot':'rightArmPivot',scene);armPivot.position.set(s*.95,4.15,0);armPivot.parent=r;rig.arms.push(armPivot);
  let arm=pcyl('arm',.34,2.1,0,-.9,0,cloth,armPivot);arm.rotation.z=s*.15;
}
if(master){
  let beard=B.MeshBuilder.CreateSphere('beard',{diameter:.9,segments:low?7:10},scene);beard.scaling.y=.7;beard.position.set(0,-.35,.38);beard.material=mats.white;beard.parent=headPivot;
  textPlane('OG MASTER',0,6.5,0,6,1.3,r);
}
r.metadata={...(r.metadata||{}),wisdoRig:rig,wisdoMaster:master};
return r}
function population(){let master=makeHuman('OG_MASTER',-185,-195,1.25,true);ogMasterNode=master;interactables.push({mesh:master.getChildMeshes()[0],label:'SPEAK WITH OG MASTER',action:()=>flash('OG MASTER • WELCOME TO THE CHAMBER')});
...
scene.collisionsEnabled=true;camera.checkCollisions=true;camera.collisionRadius=new B.Vector3(.85,.85,.85);
const fidelityRuntime=globalThis.WISDOBabylonFidelity?.install?.({B,scene,engine,camera,low})||null;
const characterRuntime=globalThis.WISDOBabylonCharacters?.install?.({B,scene,playerRoot:player,playerFallback:body,masterFallback:ogMasterNode,low})||null;
...
characterRuntime?.updatePlayer?.({speed:frameSpeed,sprinting,dt});
characterRuntime?.updateMaster?.({playerPosition:player.position,dt,action:'seated'});
fidelityRuntime?.update?.({dt,speed:frameSpeed,sprinting});
