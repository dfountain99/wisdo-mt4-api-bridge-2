...
window.addEventListener('wisdo:world-player-state',playerEvent);window.addEventListener('wisdo:game-scene',sceneEvent);window.addEventListener('wisdo:mission-step',updateWaypoint);window.addEventListener('popstate',popState);const timer=setInterval(updateWaypoint,900);ensureUi();syncSceneFlag();updateWaypoint();
window.addEventListener('pagehide',()=>{stopped=true;clearInterval(timer);window.removeEventListener('wisdo:world-player-state',playerEvent);window.removeEventListener('wisdo:game-scene',sceneEvent);window.removeEventListener('wisdo:mission-step',updateWaypoint);window.removeEventListener('popstate',popState);},{once:true});