...
export async function createWorldExperience(options = {}) {
  ...
  const telemetryProxy=(data={})=>{const operator=globalThis.WisdoOperatorDiagnostics||{},renderer=capturedRenderer;const enriched={...data,dpr:renderer?.getPixelRatio?.()??null,shadows:Boolean(renderer?.shadowMap?.enabled),rendererWidth:renderer?.domElement?.width||null,rendererHeight:renderer?.domElement?.height||null,operatorRenderer:operator.renderer||document.documentElement.dataset.wisdoOperator||'PROCEDURAL_FALLBACK',operatorStatus:operator.status||'STARTING',qualityDecision:globalThis.WisdoQualityDiagnostics?.activeQuality||data.quality||null};if(isCurrent())globalThis.WisdoRenderDiagnostics=Object.freeze(enriched);adaptive?.sample(enriched);options.onTelemetry?.(enriched);};
  ...
  const debug=new URLSearchParams(location.search).get('debug')==='1';let fidelity=null,authoredOperator=null;
  try{fidelity=await installProductionFidelity({THREE,scene:capturedScene,camera:capturedCamera,renderer:capturedRenderer,destinations:options.destinations||[],debug});if(isCurrent()){document.documentElement.dataset.wisdoFidelity='active';globalThis.WisdoFidelityStatus=Object.freeze({active:true,quality:fidelity?.quality||null,installedAt:new Date().toISOString(),renderer:'production-fidelity',instanceId});}}catch(error){if(isCurrent()){document.documentElement.dataset.wisdoFidelity='degraded';globalThis.WisdoFidelityStatus=Object.freeze({active:false,reason:error?.message||'install_failed',installedAt:new Date().toISOString(),instanceId});}console.warn('WISDO production fidelity layer degraded; core World remains active.',error);}

  if(isCurrent())document.documentElement.dataset.wisdoOperator='loading-authored-glb';
  const operatorTask=installAuthoredOperator({THREE,scene:capturedScene,debug,instanceId}).then((result)=>{if(destroyed||!isCurrent()){result?.destroy?.();return null;}authoredOperator=result;return result;}).catch((error)=>{if(!destroyed&&isCurrent()){document.documentElement.dataset.wisdoOperator='procedural-fallback';const existing=globalThis.WisdoOperatorDiagnostics||{};globalThis.WisdoOperatorDiagnostics=Object.freeze({...existing,renderer:'PROCEDURAL_FALLBACK',active:false,status:existing.status?.includes?.('FAILED')?existing.status:'FAILED',failureReason:existing.failureReason||error?.message||'authored_operator_failed',instanceId,updatedAt:new Date().toISOString()});globalThis.WisdoAuthoredAssets=Object.freeze({operator:Object.freeze({active:false,fallback:true,reason:globalThis.WisdoOperatorDiagnostics.failureReason,instanceId})});}console.warn('WISDO authored Operator unavailable; retaining procedural fallback.',error);return null;});

  const baseDestroy=core?.destroy?.bind(core),baseSetPreferences=core?.setPreferences?.bind(core);
  return {...core,visualPass:fidelity?'production-fidelity':'core-fallback',fidelityQuality:fidelity?.quality||null,get operatorRenderer(){return authoredOperator?.active?'authored-glb':document.documentElement.dataset.wisdoOperator||'procedural-fallback';},setPreferences(next={}){baseSetPreferences?.(next);if(!Object.prototype.hasOwnProperty.call(next,'quality'))return;if(next.quality==='auto'){const automatic=chooseAutoQuality();core?.setQuality?.(automatic);adaptive?.reset?.(automatic);adaptive?.setEnabled?.(true);publishQualityDiagnostics({requestedQuality:'auto',activeQuality:automatic,adaptive:true,lastReason:'user-auto'});}else{adaptive?.setEnabled?.(false);adaptive?.reset?.(next.quality);publishQualityDiagnostics({requestedQuality:next.quality,activeQuality:next.quality,adaptive:false,lastReason:'user-manual'});}},destroy(){destroyed=true;try{authoredOperator?.destroy?.();}catch(error){console.warn('Authored Operator cleanup degraded',error);}try{fidelity?.destroy?.();}catch(error){console.warn('Fidelity cleanup degraded',error);}baseDestroy?.();if(isCurrent()){delete document.documentElement.dataset.wisdoFidelity;delete document.documentElement.dataset.wisdoOperator;delete globalThis.WisdoWorldRenderInstance;}operatorTask?.catch?.(()=>{});}};
}
