...
export async function installProductionFidelity({THREE,scene,camera,renderer,debug=false}={}){
  ...
  const marketManager=createMarketBillboardManager({THREE,scene,camera,qualityName:quality,onChanged:({markets,billboardCount,assignments=[]})=>{
    globalThis.WisdoWorldDiagnostics={...(globalThis.WisdoWorldDiagnostics||{}),activeSymbols:markets.map((m)=>m.symbol),authorizedParticipants:markets.reduce((sum,m)=>sum+Number(m.activeOperatorCount||0),0),activeBillboards:billboardCount,billboardAnchorAssignments:assignments,fidelityQuality:quality,fakeCandlesAllowed:false,visualArchitecture:'production-city-core'};
  }});
  ...
  return{quality,diagnostics:globalThis.WisdoWorldDiagnostics,destroy(){destroyed=true;cancelAnimationFrame(frameId);clearInterval(timer);marketManager.destroy();}};
}
