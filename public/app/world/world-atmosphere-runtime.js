...
export function startWorldAtmosphereRuntime(){
  ...
  function apply(){if(!spatial||spatial.sceneId!=='central')return;const THREE=spatial.THREE,p=PRESETS[state.time]||PRESETS.blue_hour,visibility=WEATHER_VISIBILITY[state.weather]??1,exposure=WEATHER_EXPOSURE[state.weather]??1,sunFactor=WEATHER_SUN[state.weather]??1;spatial.scene.background=new THREE.Color(p.background);if(spatial.scene.fog){spatial.scene.fog.color.setHex(p.fog);if(Number.isFinite(original?.fogFar))spatial.scene.fog.far=original.fogFar*visibility;if(Number.isFinite(original?.fogNear))spatial.scene.fog.near=original.fogNear*(state.weather==='fog'?.7:1);}spatial.renderer.toneMappingExposure=p.exposure*exposure;const hemi=spatial.scene.children.find((o)=>o.isHemisphereLight);if(hemi){hemi.color.setHex(p.hemiSky);hemi.groundColor.setHex(p.hemiGround);hemi.intensity=p.hemi;}const sun=spatial.scene.children.find((o)=>o.isDirectionalLight);if(sun){sun.color.setHex(p.sun);sun.intensity=p.sunIntensity*sunFactor;}document.documentElement.dataset.wisdoTime=state.time;document.documentElement.dataset.wisdoWeather=state.weather;window.dispatchEvent(new CustomEvent('wisdo:world-atmosphere',{detail:{...state,decorative:true,marketSession:false}}));}
