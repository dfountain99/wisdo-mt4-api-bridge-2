...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    root().innerHTML=`<div class="mission-calendar">
      <div class="mission-space"><div class="space-stars"></div><div class="space-nebula"></div><div class="earth"><i></i></div><div class="orbit orbit-a"></div><div class="orbit orbit-b"></div><div class="station-ring"></div><div class="mission-floor"></div><div class="horizon-glow"></div></div>
      <header class="mission-instruments">
        <div class="mi"><small>SYMBOL</small><b>XAUUSD</b><span>Gold Spot</span></div><div class="mi"><small>TIMEFRAME</small><b>M5</b><span>Replay view</span></div><div class="mi live"><small>CONNECTION</small><b>${reporterFresh(acct)?'● LIVE':'○ STALE'}</b><span>Reporter</span></div><div class="mi"><small>BALANCE</small><b>${money(acct.balance||0)}</b></div><div class="mi"><small>EQUITY</small><b>${money(acct.equity||0)}</b></div><div class="mi"><small>OPEN P/L</small><b class="${Number(acct.floating_pl||0)>=0?'green':'red'}">${money(acct.floating_pl||0)}</b></div><div class="mi"><small>OPEN TRADES</small><b>${Number(acct.open_trades||0)}</b></div><div class="mi ask"><b>Ask WISDO…</b><span>◉</span></div>
      </header>
      <div class="mission-titlebar"><div><small>WISDO // COMMAND DECK</small><strong>TRADING INTELLIGENCE</strong></div><div class="deck-status"><span>FORENSIC REPLAY</span><i></i><span>LIVE ACCOUNT DATA</span></div></div><main class="mission-grid">
        <section class="mc-panel mc-calendar"><div class="mc-head"><div><small>TRADING CALENDAR</small><h3 id="calendar-month"></h3></div><div><button class="mc-arrow" id="calendar-prev">‹</button><button class="mc-arrow" id="calendar-next">›</button></div></div><div class="calendar-grid" id="calendar-grid"></div><div class="calendar-key"><span>● Profitable</span><span>● Loss</span><span>● Break Even</span></div><div class="month-stats" id="month-stats"></div></section>
        <section class="mc-panel market-theater"><div class="theater-hud"><span>MARKET PATH</span><span>ENTRY / EXIT TELEMETRY</span><span>AI FORENSICS</span></div><div class="theater-head"><div><small>FORENSIC MARKET REPLAY</small><h2 id="theater-title">XAUUSD · M5</h2><span id="theater-subtitle">Select a trading day</span></div><div class="theater-actions">◎ ◈ ⛶</div></div><div class="mission-chart idle" id="mission-chart"><div class="chart-idle"><b>SELECT A TRADING DAY</b><span>The market replay will activate here.</span></div><div class="chart-floor"></div><div class="chart-scan"></div></div></section>
        <aside class="right-stack"><section class="mc-panel trade-stats" id="trade-stats"><small>TRADE STATS</small><h3>Waiting</h3><p>Select a day from the Calendar.</p></section><section class="mc-panel progress-panel" id="progress-panel"><small>DAILY PROGRESS</small><div class="progress-ring"><b>—</b></div><p>Target not captured</p></section><section class="mc-panel context-panel" id="context-panel"><small>AI MARKET CONTEXT</small><h3>Reporter context</h3><p>Structure and session intelligence appears only when stored.</p></section></aside>
        <section class="mc-panel selected-trade" id="trade-module"><small>SELECTED TRADE</small><h3>Waiting for selection</h3><p>Tap an ENTRY or CLOSE marker.</p></section>
        <div class="wisdo-core"><div class="core-aura"></div><div class="core-rings r1"></div><div class="core-rings r2"></div><div class="core-rings r3"></div><div class="core-earth"><b>W</b></div><strong>WISDO</strong><span>ANALYZE · EXECUTE · EVOLVE</span><em>AI CORE ONLINE</em></div>
        <section class="mc-panel why-entry" id="why-module"><small>WHY THIS ENTRY?</small><h3>Decision intelligence</h3><p>WISDO will show only the reasoning actually stored with the trade.</p></section>
      </main>
      <footer class="mission-footer"><span>Overview</span><span class="active">Calendar</span><span>Trade Replay</span><span>Performance</span><span>Patterns</span><span>Insights</span><button>Analyze with AI Coach ›</button></footer>
    </div>`;
