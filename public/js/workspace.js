...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    trades.forEach(t=>{const date=String(t.closed_at||t.updated_at||t.opened_at||'').slice(0,10);if(!date)return;const d=byDay[date]||(byDay[date]={date:date,trades:[],pnl:0,wins:0,losses:0,lots:0});const p=Number(t.pnl||0);d.trades.push(t);d.pnl+=p;d.lots+=Number(t.lots||t.volume||0);if(p>0)d.wins++;else if(p<0)d.losses++;});
    const dates=Object.keys(byDay).sort(),latest=dates[dates.length-1]||new Date().toISOString().slice(0,10),acct=selectedAccount()||{};let cursor=new Date(latest+'T12:00:00'),activeDate='',events=[],idx=0,timer=null;
    root().innerHTML='<div class="memory-app"><header class="memory-top"><div><small>WISDO // TRADING MEMORY</small><h1>Campaign Calendar</h1></div><div class="memory-account"><span>'+html(acct.nickname||acct.account_number||'PORTFOLIO')+'</span><b>'+money(acct.equity||acct.balance||0)+'</b></div></header><div class="memory-layout"><aside class="memory-calendar"><div class="memory-month-head"><button id="calendar-prev">‹</button><div><small>SESSION ARCHIVE</small><h2 id="calendar-month"></h2></div><button id="calendar-next">›</button></div><div class="memory-calendar-hero"><span>PERFORMANCE CONSTELLATION</span><b>Trade days glow by outcome. Tap a day to enter its journal.</b></div><div id="calendar-grid" class="memory-grid"></div><div id="month-stats" class="memory-month-stats"></div></aside><main class="memory-theater"><header><div><small id="stage-kicker">SELECT A TRADING DAY</small><h2 id="stage-title">Campaign Memory Theater</h2><p id="stage-sub">Month → day → campaign → entry → exit → autopsy.</p></div><label class="memory-narrate"><input id="narration-mode" type="checkbox"><i></i><b>NARRATE</b></label></header><section id="mission-chart" class="memory-chart"><div class="memory-empty"><b>W</b><strong>SELECT A DAY</strong><span>WISDO will replay every recorded campaign event.</span></div></section><div class="memory-controls"><button id="story-prev" disabled>←</button><div><span id="story-count">NO CAMPAIGN</span><i><b id="story-progress"></b></i></div><button id="story-next" disabled>→</button></div><section id="story-caption" class="memory-voice"><div class="memory-orb"><b>W</b><span>WISDO</span></div><div><small>CAMPAIGN NARRATOR</small><h3>Every trade becomes a chapter.</h3><p>Select a trading day to enter its memory.</p></div></section></main><aside class="memory-inspector"><section id="trade-stats"><small>DAY MEMORY</small><h3>Nothing selected</h3></section><section id="trade-module"><small>ACTIVE CHAPTER</small><h3>—</h3></section><section id="why-module"><small>WISDO MEMORY</small><p>Stored reasoning appears here.</p></section></aside></div></div>';
    const narr=document.querySelector('#narration-mode'),nkey='wisdo_calendar_narration';narr.checked=localStorage.getItem(nkey)==='1';narr.onchange=()=>{localStorage.setItem(nkey,narr.checked?'1':'0');if(!narr.checked&&'speechSynthesis'in window)speechSynthesis.cancel();};
    const say=text=>{if(!narr.checked||!('speechSynthesis'in window))return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.rate=.9;u.pitch=.9;speechSynthesis.speak(u);};
    const stop=()=>{if(timer)clearTimeout(timer);timer=null;if('speechSynthesis'in window)speechSynthesis.cancel();};
    const drawMonth=()=>{const y=cursor.getFullYear(),m=cursor.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0),md=Object.values(byDay).filter(d=>{const z=new Date(d.date+'T12:00:00');return z.getFullYear()===y&&z.getMonth()===m});document.querySelector('#calendar-month').textContent=first.toLocaleDateString(undefined,{month:'long',year:'numeric'});let out=['S','M','T','W','T','F','S'].map(x=>'<b class="memory-dow">'+x+'</b>').join('');for(let i=0;i<first.getDay();i++)out+='<i></i>';for(let n=1;n<=last.getDate();n++){const key=y+'-'+String(m+1).padStart(2,'0')+'-'+String(n).padStart(2,'0'),d=byDay[key];out+='<button class="memory-day '+(d?(d.pnl>0?'profit':d.pnl<0?'loss':'flat'):'empty')+(key===activeDate?' selected':'')+'" data-date="'+key+'" '+(d?'':'disabled')+'><span>'+n+'</span>'+(d?'<strong>'+money(d.pnl)+'</strong><small>'+d.trades.length+' TRADES</small>':'')+'</button>';}document.querySelector('#calendar-grid').innerHTML=out;const net=md.reduce((a,d)=>a+d.pnl,0),green=md.filter(d=>d.pnl>0).length;document.querySelector('#month-stats').innerHTML='<div><b>'+md.length+'</b><span>ACTIVE DAYS</span></div><div><b>'+(md.length?Math.round(green/md.length*100):0)+'%</b><span>GREEN DAYS</span></div><div><b class="'+(net>=0?'green':'red')+'">'+money(net)+'</b><span>MONTH P/L</span></div>';document.querySelectorAll('.memory-day:not([disabled])').forEach(x=>x.onclick=()=>playDay(x.dataset.date));};
    const renderEvent=(q,i,auto)=>{idx=i;const t=q.trade,pnl=Number(t.pnl||0),side=String(t.side||'').toUpperCase(),lots=t.lots||t.volume||'—',ticket=t.ticket||t.order_id||t.id||'—',reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No entry reason was stored.',tm=new Date(q.time).toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'}),progress=events.length>1?i/(events.length-1)*100:100;document.querySelector('#story-progress').style.width=progress+'%';document.querySelector('#story-count').textContent='CHAPTER '+(i+1)+' / '+events.length;document.querySelector('#story-prev').disabled=i===0;document.querySelector('#story-next').disabled=i===events.length-1;document.querySelectorAll('.memory-node').forEach((n,j)=>{n.classList.toggle('active',j===i);n.classList.toggle('past',j<i)});const node=document.querySelector('.memory-node[data-i="'+i+'"]');if(node)node.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});const phrase=q.kind==='entry'?'Entry '+(i+1)+'. '+side+' opened at '+q.price+'. '+lots+' lots. This position eventually realized '+money(pnl)+'.':'Exit '+(i+1)+'. '+side+' closed at '+q.price+'. Realized result '+money(pnl)+'.';document.querySelector('#story-caption').innerHTML='<div class="memory-orb '+(pnl<0?'warn':'')+'"><b>'+(pnl<0?'◇':'W')+'</b><span>WISDO</span></div><div><small>'+html(tm)+' · '+q.kind.toUpperCase()+'</small><h3>'+html(side+' '+q.kind.toUpperCase())+' @ '+html(String(q.price))+'</h3><p>'+html(phrase)+'</p></div>';document.querySelector('#trade-module').innerHTML='<small>ACTIVE CHAPTER</small><h3>'+html(side+' '+q.kind.toUpperCase())+'</h3><div class="memory-facts"><span>TIME<b>'+html(tm)+'</b></span><span>PRICE<b>'+html(String(q.price))+'</b></span><span>LOTS<b>'+html(String(lots))+'</b></span><span>P/L<b class="'+(pnl>=0?'green':'red')+'">'+money(pnl)+'</b></span><span>TICKET<b>'+html(String(ticket))+'</b></span></div>';document.querySelector('#why-module').innerHTML='<small>'+(q.kind==='entry'?'WHY THIS ENTRY?':'EXIT MEMORY')+'</small><p>'+html(q.kind==='entry'?reason:'Recorded close '+q.price+'. Campaign realized '+money(pnl)+'.')+'</p><em>True candle playback will activate when Reporter stores historical OHLC.</em>';say(phrase);if(auto!==false&&i<events.length-1)timer=setTimeout(()=>renderEvent(events[i+1],i+1,true),4300);};
    const playDay=date=>{stop();activeDate=date;drawMonth();const d=byDay[date];if(!d)return;document.body.classList.add('memory-journal-open');document.querySelector('.memory-app')?.classList.add('journal-open');events=[];[...d.trades].sort((a,b)=>new Date(a.opened_at||a.updated_at)-new Date(b.opened_at||b.updated_at)).forEach(t=>{const op=Number(t.open_price),cp=Number(t.close_price),ot=new Date(t.opened_at||t.updated_at).getTime(),ct=new Date(t.closed_at||t.updated_at).getTime();if(Number.isFinite(op)&&op)events.push({kind:'entry',price:op,time:ot,trade:t});if(Number.isFinite(cp)&&cp)events.push({kind:'exit',price:cp,time:ct,trade:t})});events.sort((a,b)=>a.time-b.time);document.querySelector('#stage-kicker').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});document.querySelector('#stage-title').textContent='Day Memory · '+money(d.pnl);document.querySelector('#stage-sub').textContent=d.trades.length+' trades · '+d.lots.toFixed(2)+' lots · '+d.wins+' wins · '+d.losses+' losses';document.querySelector('#trade-stats').innerHTML='<small>DAY MEMORY</small><h3 class="'+(d.pnl>=0?'green':'red')+'">'+money(d.pnl)+'</h3><div class="memory-facts"><span>TRADES<b>'+d.trades.length+'</b></span><span>WIN RATE<b>'+(d.trades.length?Math.round(d.wins/d.trades.length*100):0)+'%</b></span><span>LOTS<b>'+d.lots.toFixed(2)+'</b></span></div>';if(!events.length){document.querySelector('#mission-chart').innerHTML='<div class="memory-empty"><strong>NO RECORDED PRICE EVENTS</strong></div>';return;}const min=Math.min(...events.map(e=>e.price)),max=Math.max(...events.map(e=>e.price)),span=Math.max(.00001,max-min),w=Math.max(820,events.length*108);let path='';const nodes=events.map((ev,i)=>{const x=62+i*((w-124)/Math.max(1,events.length-1)),y=238-(ev.price-min)/span*150;path+=(i?' L ':'M ')+x+' '+y;const p=Number(ev.trade.pnl||0);return '<button class="memory-node '+ev.kind+' '+(p>=0?'win':'loss')+'" data-i="'+i+'" style="left:'+x+'px;top:'+y+'px"><i>'+(ev.kind==='entry'?'E':'X')+(i+1)+'</i><span>'+html(new Date(ev.time).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}))+'</span></button>'}).join('');document.querySelector('#mission-chart').innerHTML='<div class="memory-map" style="width:'+w+'px"><svg viewBox="0 0 '+w+' 300" preserveAspectRatio="none"><path class="memory-glow" d="'+path+'"/><path class="memory-spine" d="'+path+'"/></svg>'+nodes+'</div>';document.querySelectorAll('.memory-node').forEach(n=>n.onclick=()=>{stop();renderEvent(events[Number(n.dataset.i)],Number(n.dataset.i),false)});document.querySelector('#story-prev').onclick=()=>{stop();const j=Math.max(0,idx-1);renderEvent(events[j],j,false)};document.querySelector('#story-next').onclick=()=>{stop();const j=Math.min(events.length-1,idx+1);renderEvent(events[j],j,false)};let close=document.querySelector('#journal-close');if(!close){close=document.createElement('button');close.id='journal-close';close.className='journal-close';close.innerHTML='← CALENDAR';document.querySelector('.memory-theater').prepend(close);}close.onclick=()=>{stop();document.body.classList.remove('memory-journal-open');document.querySelector('.memory-app')?.classList.remove('journal-open');};timer=setTimeout(()=>renderEvent(events[0],0,true),550);};
