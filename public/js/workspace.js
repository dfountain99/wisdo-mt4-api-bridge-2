...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    const keys=Object.keys(days).sort(),latest=keys[keys.length-1]||new Date().toISOString().slice(0,10);let cursor=new Date(latest+'T12:00:00'),active='',events=[],index=0,timer=null;
