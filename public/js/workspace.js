...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    const accountId=selectedAccountId(), query=accountId?`?account_id=${encodeURIComponent(accountId)}`:'';
    const result=await api(`/api/v2/trades${query}`);
    const trades=(result.trades||[]).filter(t=>t.status==='closed'&&(t.closed_at||t.updated_at||t.opened_at));
    const byDay={}; trades.forEach(t=>{const date=String(t.closed_at||t.updated_at||t.opened_at||'').slice(0,10);if(!date)return;const d=byDay[date]||=( {date,trades:[],pnl:0,wins:0,losses:0,buys:0,sells:0,lots:0} );const pnl=Number(t.pnl||0);d.trades.push(t);d.pnl+=pnl;d.lots+=Number(t.lots||t.volume||0);pnl>0?d.wins++:pnl<0?d.losses++:0;String(t.side||'').toLowerCase().includes('sell')?d.sells++:d.buys++;});
    const dates=Object.keys(byDay).sort(), latest=dates.at(-1)||new Date().toISOString().slice(0,10); let cursor=new Date(latest+'T12:00:00'), selected=null;
    const acct=selectedAccount()||{};
    root().innerHTML=`<div class="calendar-cockpit">
      <div class="cockpit-space"><i></i><i></i><i></i><div class="planet-horizon"></div></div>
      <header class="cockpit-top"><div class="cockpit-title"><span class="eyebrow">CALENDAR · TRADE REPLAY · ANALYZE · IMPROVE</span><h1>WISDO Market Observatory</h1></div><div class="instrument-rail">
        <div class="instrument"><small>ACCOUNT</small><strong>${html(acct.account_number||'—')}</strong></div><div class="instrument live"><small>REPORTER</small><strong>${reporterFresh(acct)?'● LIVE':'○ STALE'}</strong></div><div class="instrument"><small>BALANCE</small><strong>${money(acct.balance||0)}</strong></div><div class="instrument"><small>EQUITY</small><strong>${money(acct.equity||0)}</strong></div><div class="instrument"><small>FLOATING</small><strong class="${Number(acct.floating_pl||0)>=0?'green':'red'}">${money(acct.floating_pl||0)}</strong></div>
      </div></header>
      <div class="cockpit-body">
        <section class="cockpit-calendar floating-module"><div class="module-head"><div><span class="eyebrow">PERFORMANCE SCANNER</span><h3 id="calendar-month"></h3></div><div class="actions"><button class="btn ghost" id="calendar-prev">←</button><button class="btn ghost" id="calendar-next">→</button></div></div><div class="calendar-grid" id="calendar-grid"></div></section>
        <aside class="cockpit-stats floating-module" id="day-stats"><span class="eyebrow">TRADE STATS</span><div class="stat-hero">Select a day</div><p class="muted">Tap a lit day to enter its forensic deck.</p></aside>
        <section class="market-window floating-module" id="forensic-day" hidden></section>
        <section class="trade-module floating-module" id="trade-module"><span class="eyebrow">SELECTED TRADE</span><h3>Waiting for selection</h3><p class="muted">Entry, close, P/L, duration and stored reasoning will appear here.</p></section>
        <div class="campaign-core"><div class="core-orbit orbit-a"></div><div class="core-orbit orbit-b"></div><div class="core-globe"><span>W</span></div><strong>WISDO</strong><small>EXECUTE · ANALYZE · EVOLVE</small></div>
        <section class="why-module floating-module" id="why-module"><span class="eyebrow">WHY THIS ENTRY?</span><h3>Decision intelligence</h3><p class="muted">Select an entry marker. WISDO only shows reasons actually stored with the trade.</p></section>
      </div>
      <nav class="cockpit-tabs"><span>Overview</span><span class="active">Heat Map</span><span>Trade Replay</span><span>Performance</span><span>Patterns</span><span>Insights</span></nav>
    </div>`;
    const drawMonth=()=>{const y=cursor.getFullYear(),m=cursor.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0);document.querySelector('#calendar-month').textContent=first.toLocaleDateString(undefined,{month:'long',year:'numeric'});const grid=document.querySelector('#calendar-grid');let out=['SUN','MON','TUE','WED','THU','FRI','SAT'].map(x=>`<div class="calendar-dow">${x}</div>`).join('');for(let i=0;i<first.getDay();i++)out+='<div></div>';for(let n=1;n<=last.getDate();n++){const key=`${y}-${String(m+1).padStart(2,'0')}-${String(n).padStart(2,'0')}`,d=byDay[key],dir=d?(d.buys>d.sells?'↑':d.sells>d.buys?'↓':'↕'):'';out+=`<button class="calendar-day ${d?(d.pnl>=0?'profit':'loss'):'empty'}" data-date="${key}" ${d?'':'disabled'}><span>${n}</span>${d?`<strong>${money(d.pnl)}</strong><small>${d.trades.length}T · ${dir}</small>`:''}</button>`;}grid.innerHTML=out;grid.querySelectorAll('[data-date]:not([disabled])').forEach(x=>x.onclick=()=>drawDay(x.dataset.date));};
    const drawDay=date=>{selected=byDay[date];if(!selected)return;document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const winRate=selected.trades.length?Math.round(selected.wins/selected.trades.length*100):0;document.querySelector('#day-stats').innerHTML=`<span class="eyebrow">TRADE STATS</span><div class="stat-hero ${selected.pnl>=0?'green':'red'}">${money(selected.pnl)}</div><small>DAY P/L</small><div class="stat-stack"><b>${selected.trades.length}<small>Trades</small></b><b>${winRate}%<small>Win Rate</small></b><b>${selected.lots.toFixed(2)}<small>Net Lots</small></b><b>${selected.buys>selected.sells?'BUY':selected.sells>selected.buys?'SELL':'MIXED'}<small>Flow</small></b></div>`;
      const points=[];selected.trades.forEach((t,i)=>{const open=Number(t.open_price),close=Number(t.close_price);if(Number.isFinite(open)&&open)points.push({i,kind:'entry',p:open,t:new Date(t.opened_at||t.updated_at).getTime(),trade:t});if(Number.isFinite(close)&&close)points.push({i,kind:'close',p:close,t:new Date(t.closed_at||t.updated_at).getTime(),trade:t});});points.sort((x,y)=>x.t-y.t);
      let chart='<div class="no-market-data"><b>Trade path unavailable</b><span>This ledger day does not have enough stored prices to reconstruct the path.</span></div>';
      if(points.length){const minT=Math.min(...points.map(q=>q.t)),maxT=Math.max(...points.map(q=>q.t)),minP=Math.min(...points.map(q=>q.p)),maxP=Math.max(...points.map(q=>q.p)),dx=Math.max(1,maxT-minT),dp=Math.max(.00001,maxP-minP),xy=q=>[35+(q.t-minT)/dx*930,275-(q.p-minP)/dp*225];let seg='',mark='';for(let i=1;i<points.length;i++){const A=xy(points[i-1]),B=xy(points[i]),up=points[i].p>=points[i-1].p;seg+=`<line class="market-seg ${up?'up':'down'}" x1="${A[0]}" y1="${A[1]}" x2="${B[0]}" y2="${B[1]}"/>`;}points.forEach((q,i)=>{const P=xy(q),win=Number(q.trade.pnl||0)>=0,side=String(q.trade.side||'').toLowerCase().includes('sell')?'sell':'buy';mark+=q.kind==='entry'?`<g class="market-marker" data-point="${i}"><polygon class="entry ${side}" points="${P[0]},${P[1]-11} ${P[0]-9},${P[1]+8} ${P[0]+9},${P[1]+8}"/><text x="${P[0]+10}" y="${P[1]-10}">ENTRY</text></g>`:`<g class="market-marker" data-point="${i}"><circle class="close ${win?'win':'loss'}" cx="${P[0]}" cy="${P[1]}" r="8"/><text x="${P[0]+11}" y="${P[1]-10}">CLOSE</text></g>`;});chart=`<div class="thermal-field"><div class="thermal green"></div><div class="thermal red"></div></div><svg viewBox="0 0 1000 310" preserveAspectRatio="none">${seg}${mark}</svg>`;}
      const panel=document.querySelector('#forensic-day');panel.hidden=false;panel.innerHTML=`<div class="market-head"><div><span class="eyebrow">PRICE · ENTRY · PATH · CLOSE</span><h2>${new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'})}</h2></div><strong class="${selected.pnl>=0?'green':'red'}">${money(selected.pnl)}</strong></div><div class="mode-rail"><span class="active">FLOW</span><span>RISK</span><span>ENTRY QUALITY</span><span>EXPOSURE</span><span>DRAWDOWN</span><span>STRUCTURE</span></div><div class="holo-market">${chart}<div class="holo-floor"></div><div class="scan-beam"></div></div>`;
      panel.querySelectorAll('[data-point]').forEach(node=>node.onclick=()=>{const q=points[Number(node.dataset.point)],t=q.trade,pnl=Number(t.pnl||0),reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No entry reason was stored with this ledger record.';document.querySelector('#trade-module').innerHTML=`<span class="eyebrow">${pnl>=0?'WIN':'LOSS'} · ${html(String(t.side||'TRADE').toUpperCase())} · ${html(t.symbol||'')}</span><h3>${money(pnl)}</h3><div class="trade-facts"><b>Entry<small>${html(t.open_price||'—')}</small></b><b>Close<small>${html(t.close_price||'—')}</small></b><b>Lots<small>${html(t.lots||t.volume||'—')}</small></b></div>`;document.querySelector('#why-module').innerHTML=`<span class="eyebrow">WHY THIS ENTRY?</span><h3>${q.kind==='entry'?'Stored decision context':'Exit result'}</h3><p>${html(q.kind==='entry'?reason:`Closed at ${t.close_price||'—'} with P/L ${money(pnl)}`)}</p>`;});panel.scrollIntoView({behavior:'smooth',block:'center'});};
    document.querySelector('#calendar-prev').onclick=()=>{cursor=new Date(cursor.getFullYear(),cursor.getMonth()-1,1);drawMonth();};document.querySelector('#calendar-next').onclick=()=>{cursor=new Date(cursor.getFullYear(),cursor.getMonth()+1,1);drawMonth();};drawMonth(); if(byDay[latest]) drawDay(latest);
  }
