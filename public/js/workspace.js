...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    root().innerHTML=`<div class="calendar-nexus">
      ...
      <main class="nexus-layout">
        ...
        <section class="nexus-replay holo-panel">
          ...
          <div class="replay-rail"><span>◉ PRICE PATH</span><span>◇ ENTRY TELEMETRY</span><span>◆ EXIT TELEMETRY</span><span>✦ AI CONTEXT</span><button id="story-play">▶ PLAY DAY STORY</button></div><div class="story-caption" id="story-caption"><small>WISDO NARRATOR</small><strong>Select a day, then play its story.</strong><p>The replay will travel entry by entry.</p></div>
        </section>
    ...
    const drawDay=date=>{const d=byDay[date];if(!d)return;document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const wr=d.trades.length?Math.round(d.wins/d.trades.length*100):0;document.querySelector('#trade-stats').innerHTML=`<small>TRADE STATS · ${html(date)}</small><h2 class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}</h2><span>DAY P/L</span><div class="stat-quad"><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win Rate</small></b><b>${d.lots.toFixed(2)}<small>Net Lots</small></b><b>${d.buys>d.sells?'BUY':d.sells>d.buys?'SELL':'MIX'}<small>Flow</small></b></div>`;document.querySelector('#theater-subtitle').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
      const sorted=[...d.trades].sort((a,b)=>new Date(a.opened_at||a.updated_at)-new Date(b.opened_at||b.updated_at)),first=sorted[0],last=sorted.at(-1),best=sorted.reduce((a,t)=>Number(t.pnl||0)>Number(a?.pnl||-Infinity)?t:a,null),worst=sorted.reduce((a,t)=>Number(t.pnl||0)<Number(a?.pnl||Infinity)?t:a,null),dominant=d.buys>d.sells?'buyers':d.sells>d.buys?'sellers':'both sides',mood=d.pnl>0?'green':d.pnl<0?'red':'flat',opening=first?String(first.side||'').toUpperCase():'the market',closing=last?String(last.side||'').toUpperCase():'the close',arc=d.wins>d.losses?'discipline found more light than resistance':d.losses>d.wins?'the market demanded tuition before surrendering clarity':'victory and resistance held each other in balance',poem=`The day opened with ${opening} intent, and ${dominant} wrote the first line.\n\n${d.trades.length} trades crossed the tape — ${d.wins} found profit, ${d.losses} returned as lessons. ${arc}.\n\nThe strongest moment carried ${money(Number(best?.pnl||0))}; the deepest shadow reached ${money(Number(worst?.pnl||0))}. Through ${d.lots.toFixed(2)} lots of exposure, price kept speaking in movement, pressure, hesitation and release.\n\nBy the final ${closing} chapter, the session closed ${mood} at ${money(d.pnl)}. Not merely a number — a memory of where conviction held, where timing bent, and where tomorrow can become sharper.`;
      document.querySelector('#context-panel').innerHTML=`<small>WISDO AI // DAY POEM</small><h3 class="${d.pnl>=0?'green':'red'}">${d.pnl>=0?'A Day of Ascent':'A Day of Resistance'}</h3><div class="wisdo-day-poem">${poem.split('\\n\\n').map(x=>`<p>${html(x)}</p>`).join('')}</div><div class="poem-meta"><span>${d.trades.length} TRADES</span><span>${wr}% WIN RATE</span><span>${money(d.pnl)} CLOSE</span></div>`;
