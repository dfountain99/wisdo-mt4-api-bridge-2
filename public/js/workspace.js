...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    root().innerHTML = `<div class="workspace-heading"><div><span class="eyebrow">Forensic trading laboratory</span><h1>Calendar</h1><p class="muted">Scan realized performance by day. Tap a trading day to reveal its price path, entries, closes, result and stored trade context.</p></div><div class="live-chip">${trades.length} closed trades loaded</div></div>${accountMetrics(selectedAccount())}
      <div class="calendar-space-stage"><div class="space-orb orb-a"></div><div class="space-orb orb-b"></div><div class="space-horizon"></div><div class="calendar-workspace"><section class="card calendar-main floating-glass"><div class="card-head"><div><span class="eyebrow">P&L heat calendar</span><h3 id="calendar-month"></h3></div><div class="actions"><button class="btn ghost" id="calendar-prev">←</button><button class="btn ghost" id="calendar-next">→</button></div></div><div class="calendar-grid" id="calendar-grid"></div></section><aside class="card calendar-legend floating-glass"><span class="eyebrow">Heat meaning</span><h3>Read the day</h3><p><i class="heat-key profit"></i> Profit flow</p><p><i class="heat-key loss"></i> Loss flow</p><p><strong>↑ ↓ ↕</strong> dominant direction</p><p><strong>Glow</strong> = result intensity</p><p class="muted">The forensic chart stays hidden until you select a trading day.</p></aside></div><section class="card forensic-day floating-glass replay-deck" id="forensic-day" hidden></section>`;
    const drawMonth = () => {
    ...
    const drawDay = (date) => {
      ...
      if(points.length){const minT=Math.min(...points.map(p=>p.t)),maxT=Math.max(...points.map(p=>p.t)),minP=Math.min(...points.map(p=>p.p)),maxP=Math.max(...points.map(p=>p.p)),dx=Math.max(1,maxT-minT),dp=Math.max(.00001,maxP-minP),xy=q=>[45+(q.t-minT)/dx*910,270-(q.p-minP)/dp*220];let lines='',marks='';for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i],A=xy(a),B=xy(b),up=b.p>=a.p;lines+=`<line class="forensic-price ${up?'up':'down'}" x1="${A[0]}" y1="${A[1]}" x2="${B[0]}" y2="${B[1]}"/>`;}points.forEach((q,i)=>{const P=xy(q),win=Number(q.trade.pnl||0)>=0,side=String(q.trade.side||'').toLowerCase().includes('sell')?'sell':'buy';marks+=q.kind==='entry'?`<g class="forensic-marker" data-point="${i}"><polygon class="entry ${side}" points="${P[0]},${P[1]-9} ${P[0]-8},${P[1]+7} ${P[0]+8},${P[1]+7}"/><text x="${P[0]+10}" y="${P[1]-8}">ENTRY</text></g>`:`<g class="forensic-marker" data-point="${i}"><circle class="close ${win?'win':'loss'}" cx="${P[0]}" cy="${P[1]}" r="7"/><text x="${P[0]+10}" y="${P[1]-8}">CLOSE ${win?'W':'L'}</text></g>`;});chart=`<div class="price-heat-chart holographic-chart"><div class="chart-depth-floor"></div><svg viewBox="0 0 1000 310" preserveAspectRatio="none">${lines}${marks}</svg></div>`;}
