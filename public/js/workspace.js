...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    document.body.classList.remove('memory-journal-open');
    const accountId=selectedAccountId(),query=accountId?'?account_id='+encodeURIComponent(accountId):'',result=await api('/api/v2/trades'+query),acct=selectedAccount()||{};
    const trades=(result.trades||[]).filter(t=>t.status==='closed'&&(t.closed_at||t.updated_at||t.opened_at)),days={};
    trades.forEach(t=>{const key=String(t.closed_at||t.updated_at||t.opened_at||'').slice(0,10);if(!key)return;const d=days[key]||(days[key]={date:key,trades:[],pnl:0,wins:0,losses:0,lots:0});const p=Number(t.pnl||0);d.trades.push(t);d.pnl+=p;d.lots+=Number(t.lots||t.volume||0);if(p>0)d.wins++;else if(p<0)d.losses++;});
    const keys=Object.keys(days).sort(),latest=keys[keys.length-1]||new Date().toISOString().slice(0,10);let cursor=new Date(latest+'T12:00:00'),active='',events=[],index=0,timer=null,playing=false;
    root().innerHTML='<div class="nova-calendar"><header class="nova-head"><div><small>WISDO // TEMPORAL JOURNAL</small><h1>Trading Calendar</h1><p>Every trading day is a memory you can enter.</p></div><div class="nova-balance"><span>'+html(acct.nickname||acct.account_number||'PORTFOLIO')+'</span><b>'+money(acct.equity||acct.balance||0)+'</b></div></header><section class="nova-month"><div class="nova-monthbar"><button id="calendar-prev">‹</button><div><small>PERFORMANCE CONSTELLATION</small><h2 id="calendar-month"></h2></div><button id="calendar-next">›</button></div><div id="calendar-grid" class="nova-grid"></div><footer id="month-stats" class="nova-stats"></footer></section><section class="nova-preview"><div><small>DAY JOURNAL</small><h2 id="preview-title">Select a glowing day</h2><p id="preview-copy">Tap any recorded session to open its full-screen trading story.</p></div><div class="nova-orb">W</div></section></div>';
    const renderMonth=()=>{const y=cursor.getFullYear(),m=cursor.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0),md=Object.values(days).filter(d=>{const z=new Date(d.date+'T12:00:00');return z.getFullYear()===y&&z.getMonth()===m});document.querySelector('#calendar-month').textContent=first.toLocaleDateString(undefined,{month:'long',year:'numeric'});let out=['SUN','MON','TUE','WED','THU','FRI','SAT'].map(x=>'<b class="nova-dow">'+x+'</b>').join('');for(let i=0;i<first.getDay();i++)out+='<i></i>';for(let n=1;n<=last.getDate();n++){const key=y+'-'+String(m+1).padStart(2,'0')+'-'+String(n).padStart(2,'0'),d=days[key];out+='<button class="nova-day '+(d?(d.pnl>0?'profit':d.pnl<0?'loss':'flat'):'empty')+(key===active?' selected':'')+'" data-date="'+key+'" '+(d?'':'disabled')+'><span>'+n+'</span>'+(d?'<i></i><strong>'+money(d.pnl)+'</strong><small>'+d.trades.length+' trades · '+Math.round(d.wins/Math.max(1,d.trades.length)*100)+'%</small>':'<small>NO MEMORY</small>')+'</button>';}document.querySelector('#calendar-grid').innerHTML=out;const net=md.reduce((a,d)=>a+d.pnl,0),green=md.filter(d=>d.pnl>0).length,total=md.reduce((a,d)=>a+d.trades.length,0);document.querySelector('#month-stats').innerHTML='<div><span>ACTIVE DAYS</span><b>'+md.length+'</b></div><div><span>TRADES</span><b>'+total+'</b></div><div><span>GREEN DAYS</span><b>'+(md.length?Math.round(green/md.length*100):0)+'%</b></div><div><span>MONTH P/L</span><b class="'+(net>=0?'green':'red')+'">'+money(net)+'</b></div>';document.querySelectorAll('.nova-day:not([disabled])').forEach(n=>{n.onmouseenter=()=>{const d=days[n.dataset.date];document.querySelector('#preview-title').textContent=new Date(d.date+'T12:00:00').toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});document.querySelector('#preview-copy').textContent=d.trades.length+' trades · '+money(d.pnl)+' · tap to enter journal';};n.onclick=()=>openJournal(n.dataset.date)});};
    const stop=()=>{if(timer)clearTimeout(timer);timer=null;playing=false;if('speechSynthesis'in window)speechSynthesis.cancel();};
    const speak=text=>{const toggle=document.querySelector('#journal-narration');if(!toggle||!toggle.checked||!('speechSynthesis'in window))return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.rate=.9;u.pitch=.9;speechSynthesis.speak(u);};
    const chapter=(i,auto=true)=>{index=i;const e=events[i],t=e.trade,pnl=Number(t.pnl||0),side=String(t.side||'').toUpperCase(),lots=t.lots||t.volume||'—',tm=new Date(e.time).toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'}),reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No entry reason was stored.',prev=events[i-1],delta=prev?e.price-prev.price:0,direction=delta>0?'rose':delta<0?'fell':'held its ground',movement=prev?Math.abs(delta).toFixed(2):'0.00',chapterName=e.kind==='entry'?'The Door Opens':'The Market Answers',phrase=e.kind==='entry'?'At '+tm+', '+side+' stepped into gold at '+e.price+', carrying '+lots+' lots into the unknown. '+(prev?'Since the last remembered moment, price '+direction+' '+movement+'; another footprint joined the campaign.':'This was the first footprint in this remembered campaign.')+' WISDO marks the moment, not as a number alone, but as the place where intention met the market.':'At '+tm+', the '+side+' chapter released at '+e.price+'. The market returned '+money(pnl)+' to the ledger. '+(prev?'From the prior remembered point, price '+direction+' '+movement+'. ':'')+(pnl>=0?'What was once risk became harvest; the path can now be studied for what it gave.':'The loss becomes a landmark, a place to study what changed before the campaign let go.');document.querySelectorAll('.journal-node').forEach((n,j)=>{n.classList.toggle('active',j===i);n.classList.toggle('past',j<i)});document.querySelector('#journal-progress').style.width=(events.length>1?i/(events.length-1)*100:100)+'%';const play=document.querySelector('#journal-play');if(play)play.textContent=playing?'Ⅱ':'▶';document.querySelector('#journal-count').textContent='CHAPTER '+(i+1)+' / '+events.length;document.querySelector('#journal-voice').innerHTML='<small>'+html(tm)+' · '+e.kind.toUpperCase()+' · '+html(chapterName.toUpperCase())+'</small><h3>'+html(chapterName)+'</h3><p>'+html(phrase)+'</p>';document.querySelector('#journal-detail').innerHTML='<small>ACTIVE MEMORY</small><div><span>PRICE<b>'+html(String(e.price))+'</b></span><span>LOTS<b>'+html(String(lots))+'</b></span><span>P/L<b class="'+(pnl>=0?'green':'red')+'">'+money(pnl)+'</b></span></div><p>'+html(e.kind==='entry'?reason:'Recorded campaign exit.')+'</p>';const node=document.querySelector('.journal-node[data-i="'+i+'"]');if(node)node.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});speak(phrase);if(auto&&i<events.length-1){playing=true;timer=setTimeout(()=>chapter(i+1,true),4200);}else if(i>=events.length-1){playing=false;const play=document.querySelector('#journal-play');if(play)play.textContent='↻';}};
    const openJournal=date=>{stop();active=date;renderMonth();const d=days[date];events=[];d.trades.forEach(t=>{const op=Number(t.open_price),cp=Number(t.close_price),ot=new Date(t.opened_at||t.updated_at).getTime(),ct=new Date(t.closed_at||t.updated_at).getTime();if(Number.isFinite(op)&&op)events.push({kind:'entry',price:op,time:ot,trade:t});if(Number.isFinite(cp)&&cp)events.push({kind:'exit',price:cp,time:ct,trade:t})});events.sort((a,b)=>a.time-b.time);document.body.classList.add('memory-journal-open');const shell=document.createElement('div');shell.className='day-journal';const label=new Date(date+'T12:00:00').toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric',year:'numeric'});const min=events.length?Math.min(...events.map(e=>e.price)):0,max=events.length?Math.max(...events.map(e=>e.price)):1,span=Math.max(.00001,max-min),w=Math.max(900,events.length*115);let path='';const nodes=events.map((e,i)=>{const x=70+i*((w-140)/Math.max(1,events.length-1)),y=250-(e.price-min)/span*165;path+=(i?' L ':'M ')+x+' '+y;return '<button class="journal-node '+e.kind+'" data-i="'+i+'" style="left:'+x+'px;top:'+y+'px"><i>'+(e.kind==='entry'?'E':'X')+(i+1)+'</i><span>'+html(new Date(e.time).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}))+'</span></button>'}).join('');shell.innerHTML='<header><button id="journal-close">← CALENDAR</button><div><small>WISDO // DAY MEMORY</small><h1>'+html(label)+'</h1><p>'+d.trades.length+' trades · '+d.lots.toFixed(2)+' lots · <b class="'+(d.pnl>=0?'green':'red')+'">'+money(d.pnl)+'</b></p></div><label><input type="checkbox" id="journal-narration"><i></i><span>NARRATION</span></label></header><main><section class="journal-stage"><div class="journal-map" style="width:'+w+'px"><svg viewBox="0 0 '+w+' 320" preserveAspectRatio="none"><path class="journal-glow" d="'+path+'"/><path class="journal-spine" d="'+path+'"/></svg>'+nodes+'</div></section><div class="journal-transport"><button id="journal-prev">‹</button><button id="journal-play" class="journal-play">▶</button><div><span id="journal-count">CAMPAIGN READY</span><i><b id="journal-progress"></b></i></div><button id="journal-next">›</button></div><section class="journal-narrator"><div class="journal-wisdo">W</div><div id="journal-voice"><small>WISDO NARRATOR</small><h3>Entering the trading day.</h3><p>The journal will move through each recorded event.</p></div></section></main><aside id="journal-detail"><small>DAY SUMMARY</small><h2 class="'+(d.pnl>=0?'green':'red')+'">'+money(d.pnl)+'</h2><div><span>TRADES<b>'+d.trades.length+'</b></span><span>WIN RATE<b>'+Math.round(d.wins/Math.max(1,d.trades.length)*100)+'%</b></span><span>LOTS<b>'+d.lots.toFixed(2)+'</b></span></div><p>Only stored trade events are shown. Historical candle replay requires OHLC capture.</p></aside>';document.body.appendChild(shell);const nt=document.querySelector('#journal-narration');nt.checked=localStorage.getItem('wisdo_calendar_narration')==='1';nt.onchange=()=>localStorage.setItem('wisdo_calendar_narration',nt.checked?'1':'0');document.querySelector('#journal-close').onclick=()=>{stop();shell.remove();document.body.classList.remove('memory-journal-open')};document.querySelectorAll('.journal-node').forEach(n=>n.onclick=()=>{stop();chapter(Number(n.dataset.i),false)});document.querySelector('#journal-prev').onclick=()=>{stop();chapter(Math.max(0,index-1),false)};document.querySelector('#journal-next').onclick=()=>{stop();chapter(Math.min(events.length-1,index+1),false)};document.querySelector('#journal-play').onclick=()=>{if(!events.length)return;if(playing){stop();document.querySelector('#journal-play').textContent='▶';return;}playing=true;if(index>=events.length-1)index=0;document.querySelector('#journal-play').textContent='Ⅱ';chapter(index,true)};if(events.length){playing=true;timer=setTimeout(()=>chapter(0,true),650);}};
    document.querySelector('#calendar-prev').onclick=()=>{cursor=new Date(cursor.getFullYear(),cursor.getMonth()-1,1);renderMonth()};document.querySelector('#calendar-next').onclick=()=>{cursor=new Date(cursor.getFullYear(),cursor.getMonth()+1,1);renderMonth()};renderMonth();
  }
