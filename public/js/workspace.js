...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    const drawDay=date=>{const d=byDay[date];if(!d)return;document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const wr=d.trades.length?Math.round(d.wins/d.trades.length*100):0;document.querySelector('#trade-stats').innerHTML=`<small>TRADE STATS · ${html(date)}</small><h2 class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}</h2><span>DAY P/L</span><div class="stat-quad"><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win Rate</small></b><b>${d.lots.toFixed(2)}<small>Net Lots</small></b><b>${d.buys>d.sells?'BUY':d.sells>d.buys?'SELL':'MIX'}<small>Flow</small></b></div>`;document.querySelector('#theater-subtitle').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
      ...
      const mc=document.querySelector('#mission-chart');mc.classList.remove('idle');mc.innerHTML=chart+'<div class="chart-floor"></div><div class="chart-scan"></div><div class="chapter-stage" id="chapter-stage"></div>';const caption=document.querySelector('#story-caption'),narrationToggle=document.querySelector('#narration-mode');if(window.__wisdoStoryTimer){clearInterval(window.__wisdoStoryTimer);window.__wisdoStoryTimer=null;}if('speechSynthesis'in window)window.speechSynthesis.cancel();let storyTimer=null,storyIndex=0,spoken=null;const narrationKey='wisdo_calendar_narration';if(narrationToggle){narrationToggle.checked=localStorage.getItem(narrationKey)==='1';narrationToggle.onchange=()=>{localStorage.setItem(narrationKey,narrationToggle.checked?'1':'0');if(!narrationToggle.checked){if('speechSynthesis'in window)window.speechSynthesis.cancel();window.__wisdoNarrationAudio?.pause?.();}};}const memoryOf=t=>{const values=[t.entry_reason,t.signal_reason,t.reason,t.comment,t.strategy,t.bot_comment,t.decision_reason,t.setup,t.signal].filter(v=>String(v||'').trim()).map(v=>String(v).trim());return [...new Set(values)].join(' · ');};
      const lessonFor=(q,index)=>{const prev=points[index-1],next=points[index+1],side=String(q.trade?.side||'').toLowerCase(),pnl=Number(q.trade?.pnl||0);if(!prev)return 'The first position is a hypothesis. Price must still earn our conviction.';const move=Number(q.p)-Number(prev.p),aligned=(side.includes('buy')&&move>0)||(side.includes('sell')&&move<0);if(q.kind==='entry'&&!aligned)return 'Notice this carefully. The new position arrived against the most recent recorded movement. A better price and a broken thesis can look remarkably similar until structure separates them.';if(q.kind==='entry'&&aligned)return 'Momentum is cooperating, but cooperation is not permission to become careless. Every add increases both opportunity and obligation.';if(q.kind==='close'&&pnl>0)return 'Profit is the receipt, not the proof. The lesson is to identify which decision created the advantage and which merely survived long enough to share it.';if(q.kind==='close'&&pnl<0)return 'Do not look away from the red ink. Walk backward and find the earliest recorded moment that challenged the original idea; that is where the education begins.';if(next){const nextMove=Number(next.p)-Number(q.p);if(Math.abs(nextMove)>Math.abs(move)*1.5)return 'What follows is expansion. Remember this location; repeated acceptance or rejection around the same kind of structure can become a pattern worth studying.';}return 'One recorded point is information. Sequence gives it meaning. Watch what price does next before deciding what this moment truly was.';};
      const buildCampaignScript=()=>points.map((q,index)=>{const t=q.trade||{},side=String(t.side||'').toUpperCase()||'POSITION',tm=new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),prev=points[index-1],move=prev?Number(q.p)-Number(prev.p):0,drift=move>0?'lifted':move<0?'fell':'held near the same level',memory=memoryOf(t),pnl=Number(t.pnl||0),lesson=lessonFor(q,index);let narration;if(q.kind==='entry'){narration=index===0?`At ${tm}, the room grew quiet. ${side} stepped from thought into consequence. ${memory?`The machine left us this memory: “${memory}.” `:''}Do not mistake an entry for certainty. It is merely a thesis asking the market to answer. ${lesson}`:`By ${tm}, price had ${drift} ${Math.abs(move).toFixed(2)} from our last recorded memory. Another ${side} footprint entered the road. ${memory?`The machine remembered: “${memory}.” `:''}Now the burden changes with the average. ${lesson}`;}else{narration=pnl>=0?`At ${tm}, the campaign released ${money(pnl)} from the movement. ${memory?`Its recorded memory was: “${memory}.” `:''}But green numbers can flatter poor decisions, so we will study the road and not merely applaud the destination. ${lesson}`:`At ${tm}, the ledger accepted ${money(Math.abs(pnl))} in tuition. ${memory?`The machine carried this memory: “${memory}.” `:''}Loss becomes wisdom only when we discover where reasoning and price first parted company. ${lesson}`;}return{q,index,narration,lesson,memory,state:q.kind==='close'?(pnl>=0?'HARVEST':'AUTOPSY'):(index?'ATTACK':'OBSERVING'),label:q.kind==='entry'?(index?'ADD':'ENTRY'):(pnl>=0?'HARVEST':'LESSON')};});
      let campaignScript=buildCampaignScript(),voiceState='READY',storyPaused=false;
