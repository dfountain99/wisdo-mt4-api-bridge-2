...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    const drawDay=date=>{const d=byDay[date];if(!d)return;document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const wr=d.trades.length?Math.round(d.wins/d.trades.length*100):0;document.querySelector('#trade-stats').innerHTML=`<small>TRADE STATS · ${html(date)}</small><h2 class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}</h2><span>DAY P/L</span><div class="stat-quad"><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win Rate</small></b><b>${d.lots.toFixed(2)}<small>Net Lots</small></b><b>${d.buys>d.sells?'BUY':d.sells>d.buys?'SELL':'MIX'}<small>Flow</small></b></div>`;document.querySelector('#theater-subtitle').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
      ...
      const mc=document.querySelector('#mission-chart');mc.classList.remove('idle');mc.innerHTML=chart+'<div class="chart-floor"></div><div class="chart-scan"></div><div class="chapter-stage" id="chapter-stage"></div>';const caption=document.querySelector('#story-caption'),narrationToggle=document.querySelector('#narration-mode');if(window.__wisdoStoryTimer){clearInterval(window.__wisdoStoryTimer);window.__wisdoStoryTimer=null;}if('speechSynthesis'in window)window.speechSynthesis.cancel();let storyTimer=null,storyIndex=0,spoken=null;const narrationKey='wisdo_calendar_narration';if(narrationToggle){narrationToggle.checked=localStorage.getItem(narrationKey)==='1';narrationToggle.onchange=()=>{localStorage.setItem(narrationKey,narrationToggle.checked?'1':'0');if(!narrationToggle.checked&&'speechSynthesis'in window)window.speechSynthesis.cancel();};}const speak=text=>{if(!narrationToggle?.checked||!('speechSynthesis'in window)||!text)return;window.speechSynthesis.cancel();spoken=new SpeechSynthesisUtterance(text);spoken.rate=.88;spoken.pitch=.88;spoken.volume=1;const voices=window.speechSynthesis.getVoices(),voice=voices.find(v=>/en-US/i.test(v.lang)&&/male|david|daniel|aaron|alex|fred/i.test(v.name))||voices.find(v=>/en-US/i.test(v.lang))||voices[0];if(voice)spoken.voice=voice;window.speechSynthesis.speak(spoken);};const tradeThought=t=>{const raw=[t.entry_reason,t.signal_reason,t.reason,t.comment,t.strategy,t.magic_comment,t.bot_comment,t.decision_reason,t.setup,t.signal].filter(Boolean).map(String);return [...new Set(raw)].join(' · ')||'The bot left no written thought for this moment.';};
      const priceLesson=(q,index)=>{const prev=points[index-1],next=points[index+1],t=q.trade,pnl=Number(t.pnl||0);if(!prev)return 'Lesson: the first decision is a hypothesis, not a promise. Let the next price action confirm whether the market accepts it.';const move=Number(q.p)-Number(prev.p),priorSide=String(t.side||'').toLowerCase(),withTrade=(priorSide.includes('buy')&&move>0)||(priorSide.includes('sell')&&move<0);if(q.kind==='entry'&&withTrade)return 'Lesson: price is moving with the position. An add should still earn its place through structure and acceptance; profit alone is not confirmation.';if(q.kind==='entry'&&!withTrade)return 'Lesson: this add arrived against the recent move. That is where discipline matters most—distinguish a planned better price from averaging into invalidation.';if(q.kind==='close'&&pnl>0)return 'Lesson: the exit proves realized profit, not that every decision was correct. Study which entries created the edge and which merely survived the move.';if(q.kind==='close'&&pnl<0)return 'Lesson: a losing close is useful evidence. Look backward for the first price-action change that contradicted the original thesis; that moment matters more than the final loss.';if(next&&Math.abs(Number(next.p)-Number(q.p))>Math.abs(move)*1.5)return 'Lesson: expansion followed this point. Mark what price accepted or rejected here; repeated behavior around the same structure can become a future teaching pattern.';return 'Lesson: watch the relationship between structure, timing, and the next reaction. One candle is information; sequence gives it meaning.';};
      const poeticChapter=(q,index)=>{const t=q.trade,pnl=Number(t.pnl||0),side=String(t.side||'').toUpperCase(),tm=new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),prev=points[index-1],shift=prev?Number(q.p)-Number(prev.p):0,drift=shift>0?'climbed':shift<0?'slipped lower':'stood nearly still',distance=Math.abs(shift).toFixed(2),thought=tradeThought(t),lesson=priceLesson(q,index);let story;if(q.kind==='entry'){story=index===0?`At ${tm}, the room grew quiet. ${side} made the first move, and the campaign stepped from thought into consequence. The machine's own note was: “${thought}” I would not call that certainty, son—only a thesis asking price to answer it.`:`By ${tm}, price had ${drift} ${distance} from the last remembered point. Another ${side} footprint entered the road. The machine wrote: “${thought}” Now watch closely: every added position changes both the average and the burden. The question is no longer simply where price goes, but whether the reason for staying remains alive.`;}else{story=pnl>=0?`At ${tm}, the market paid the campaign ${money(pnl)}. That green number is the ending, not the whole truth. The machine carried this note through the trade: “${thought}” A seasoned eye still asks which decision earned the harvest and which one was merely rescued by the tide.`:`At ${tm}, the ledger took ${money(Math.abs(pnl))}. Do not rush past the red ink. The machine's remembered thought was: “${thought}” Loss is most valuable when we walk backward through price and find the first place the thesis stopped being true.`;}return story+' '+lesson;};
      const narrate=(q,index)=>{const t=q.trade,pnl=Number(t.pnl||0),side=String(t.side||'').toUpperCase(),price=q.kind==='entry'?t.open_price:t.close_price,node=mc.querySelector(`[data-point="${index}"]`);mc.querySelectorAll('.trade-marker').forEach(n=>n.classList.remove('story-focus'));node?.classList.add('story-focus');if(node){const circle=node.querySelector('circle'),svg=mc.querySelector('svg'),cx=Number(circle?.getAttribute('cx')||500),cy=Number(circle?.getAttribute('cy')||155),fx=(cx/1000*100).toFixed(2),fy=(cy/310*100).toFixed(2);svg?.style.setProperty('--focus-x',fx+'%');svg?.style.setProperty('--focus-y',fy+'%');svg?.classList.remove('story-riding');requestAnimationFrame(()=>requestAnimationFrame(()=>svg?.classList.add('story-riding')));mc.style.setProperty('--story-glow',pnl>=0?'rgba(54,255,179,.24)':'rgba(255,76,112,.22)');}const stage=mc.querySelector('#chapter-stage'),opened=new Date(t.opened_at||t.updated_at),closed=new Date(t.closed_at||t.updated_at),duration=Number.isNaN(opened.getTime())||Number.isNaN(closed.getTime())?'—':Math.max(0,Math.round((closed-opened)/60000))+'m',reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No stored entry reason',expression=pnl>0?'✦':pnl<0?'◇':'◉';if(stage){stage.classList.remove('chapter-in');stage.innerHTML=`<div class="wisdo-expression ${pnl>=0?'positive':'negative'}"><b>${expression}</b><span>WISDO</span></div><div class="chapter-readout"><small>CAMPAIGN EVENT ${index+1} / ${points.length}</small><strong>${html(side)} · ${q.kind.toUpperCase()}</strong><div><span>TIME <b>${html(new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'}))}</b></span><span>PRICE <b>${html(String(price||'—'))}</b></span><span>LOTS <b>${html(String(t.lots||t.volume||'—'))}</b></span><span>P/L <b>${money(pnl)}</b></span><span>HOLD <b>${html(duration)}</b></span></div><p>${html(q.kind==='entry'?reason:'Campaign result recorded at this exit.')}</p><em>OHLC candle replay requires Reporter candle history; WISDO will not invent missing candles.</em></div>`;requestAnimationFrame(()=>stage.classList.add('chapter-in'));}caption.innerHTML=`<small>CHAPTER ${index+1} / ${points.length} · ${q.kind.toUpperCase()}</small><strong>${q.kind==='entry'?`${side} entered at ${price||'—'}`:`${side} closed at ${price||'—'}`}</strong><p>${q.kind==='entry'?`WISDO follows the recorded trade event. This position would finish ${pnl>=0?'in profit':'in loss'} at ${money(pnl)}.`:`${pnl>=0?'The campaign realized profit here.':'The campaign realized a loss here.'} The review continues to the next recorded event.`}</p>`;const spokenChapter=poeticChapter(q,index);caption.querySelector('p').textContent=spokenChapter;speak(spokenChapter);node?.dispatchEvent(new Event('click'));};const stopStory=()=>{if(window.__wisdoStoryTimer){clearTimeout(window.__wisdoStoryTimer);window.__wisdoStoryTimer=null;}storyTimer=null;};const finishStory=()=>{stopStory();mc.querySelector('svg')?.classList.remove('story-riding');mc.classList.remove('story-live');caption.innerHTML+=`<em>DAY STORY COMPLETE · ${money(d.pnl)} · WISDO MEMORY STORED</em>`;speak(`The day story is complete. The session closed at ${money(d.pnl)}. WISDO memory stored.`);};const advanceStory=()=>{if(storyIndex>=points.length){finishStory();return;}narrate(points[storyIndex],storyIndex);storyIndex++;window.__wisdoStoryTimer=setTimeout(advanceStory,4200);storyTimer=window.__wisdoStoryTimer;};const startStory=()=>{stopStory();storyIndex=0;if(!points.length)return;mc.classList.add('story-live');advanceStory();};window.__wisdoStoryTimer=setTimeout(startStory,700);storyTimer=window.__wisdoStoryTimer;
