(() => {
  ...
  async function drawAccounts() {
    ...
    root().innerHTML = `
      <div class="workspace-heading"><div><span class="eyebrow">Database-backed connections</span><h1>Trading Accounts</h1><p class="muted">Connect with Reporter v1.58, MetaApi, cTrader OAuth, or a signed broker webhook. All identities, tokens, snapshots, and lane membership are stored in PostgreSQL—never local JSON files.</p></div><div class="actions"><button class="btn primary" id="add-account">Reporter / manual</button><button class="btn gold" id="add-broker-api">Broker API</button></div></div>
