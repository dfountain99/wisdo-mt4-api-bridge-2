...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    ...
    const drawDay=date=>{const d=byDay[date];if(!d)return;document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const wr=d.trades.length?Math.round(d.wins/d.trades.length*100):0;document.querySelector('#trade-stats').innerHTML=`<small>TRADE STATS · ${html(date)}</small><h2 class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}</h2><span>DAY P/L</span><div class="stat-quad"><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win Rate</small></b><b>${d.lots.toFixed(2)}<small>Net Lots</small></b><b>${d.buys>d.sells?'BUY':d.sells>d.buys?'SELL':'MIX'}<small>Flow</small></b></div>`;document.querySelector('#theater-subtitle').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
      ...
      const speak=text=>new Promise(async resolve=>{if(!narrationToggle?.checked||!text){resolve();return;}setVoiceState('GENERATING');try{const response=await fetch('/api/wisdo/voice/speak',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'audio/mpeg'},body:JSON.stringify({text})});if(!response.ok)throw new Error('WISDO voice endpoint '+response.status);if(!String(response.headers.get('content-type')||'').includes('audio'))throw new Error('WISDO voice returned non-audio');const blob=await response.blob(),url=URL.createObjectURL(blob),audio=new Audio(url);setVoiceState('SPEAKING');window.__wisdoNarrationAudio?.pause?.();window.__wisdoNarrationAudio=audio;const done=()=>{URL.revokeObjectURL(url);if(window.__wisdoNarrationAudio===audio)window.__wisdoNarrationAudio=null;setVoiceState('READY');resolve();};audio.onended=done;audio.onerror=()=>{URL.revokeObjectURL(url);setVoiceState('OFFLINE','AUDIO');resolve();};await audio.play();}catch(error){console.warn('WISDO cinematic voice unavailable',error);setVoiceState('OFFLINE','ENDPOINT');resolve();}});
