...
${result.secret}`)}catch(error){status.className='form-status error';status.textContent=error.message} };
  ...
  async function drawCalendar() {
    const accountId=selectedAccountId(),query=accountId?`?account_id=${encodeURIComponent(accountId)}`:'',result=await api(`/api/v2/trades${query}`);
    const trades=(result.trades||[]).filter(t=>t.status==='closed'&&(t.closed_at||t.updated_at||t.opened_at)),byDay={};
    trades.forEach(t=>{const date=String(t.closed_at||t.updated_at||t.opened_at||'').slice(0,10);if(!date)return;const d=byDay[date]||=( {date,trades:[],pnl:0,wins:0,losses:0,buys:0,sells:0,lots:0} ),p=Number(t.pnl||0);d.trades.push(t);d.pnl+=p;d.lots+=Number(t.lots||t.volume||0);p>0?d.wins++:p<0?d.losses++:0;String(t.side||'').toLowerCase().includes('sell')?d.sells++:d.buys++;});
    const dates=Object.keys(byDay).sort(),latest=dates.at(-1)||new Date().toISOString().slice(0,10),acct=selectedAccount()||{};let cursor=new Date(latest+'T12:00:00');
    let calendarItems=[];try{const feed=await api('/api/v2/calendar/items');calendarItems=feed.items||[];}catch(error){console.warn('Living Calendar feed unavailable',error);}const itemsByDay={};calendarItems.forEach(item=>{const day=String(item.date||item.startsAt||'').slice(0,10);if(day)(itemsByDay[day]||=[]).push(item);});
    root().innerHTML=`<div class="calendar-nexus">
      <div class="nexus-world"><div class="nexus-stars"></div><div class="nexus-aurora"></div><div class="nexus-planet"></div><div class="nexus-orbit o1"></div><div class="nexus-orbit o2"></div><div class="nexus-gridfloor"></div></div>
      <header class="nexus-top">
        <div class="nexus-brand"><span class="nexus-sigil">W</span><div><small>WISDO // TEMPORAL INTELLIGENCE</small><strong>CALENDAR NEXUS</strong></div></div>
        <div class="nexus-live"><i></i><span>${reporterFresh(acct)?'REPORTER LIVE':'REPORTER STALE'}</span></div>
        <div class="nexus-metrics"><b><small>BALANCE</small>${money(acct.balance||0)}</b><b><small>EQUITY</small>${money(acct.equity||0)}</b><b><small>FLOATING</small><span class="${Number(acct.floating_pl||0)>=0?'green':'red'}">${money(acct.floating_pl||0)}</span></b><b><small>OPEN</small>${Number(acct.open_trades||0)}</b></div>
      </header>
      <main class="nexus-layout">
        <section class="nexus-calendar holo-panel">
          <div class="nexus-panel-head"><div><small>MONTH MATRIX</small><h2 id="calendar-month"></h2></div><div class="nexus-month-nav"><button id="calendar-prev">‹</button><button id="calendar-next">›</button></div></div>
          <div class="calendar-grid" id="calendar-grid"></div>
          <div class="calendar-key"><span>● PROFIT</span><span>● LOSS</span><span>● BREAK EVEN</span></div>
          <div class="month-stats" id="month-stats"></div>
        </section>
        <section class="nexus-replay holo-panel">
          <div class="replay-command"><div><small>TRADE MEMORY // XAUUSD</small><h2 id="theater-title">MARKET REPLAY</h2><span id="theater-subtitle">Choose a trading day to reconstruct the session</span></div><div class="replay-chip">M5 · FORENSIC</div></div>
          <div class="mission-chart idle" id="mission-chart"><div class="chart-idle"><span class="idle-orb">W</span><b>AWAITING DAY SELECTION</b><span>Review center · hourly wage lab · decision memory · notes</span></div><div class="chart-floor"></div><div class="chart-scan"></div></div>
          <div class="review-center" id="review-center" hidden></div>
          <div class="replay-rail"><span>◉ PRICE PATH</span><span>◇ ENTRY TELEMETRY</span><span>◆ EXIT TELEMETRY</span><span>✦ AI NARRATIVE</span><span class="auto-story">● AUTO STORY</span><span class="wisdo-voice-state" id="wisdo-voice-state" data-state="READY">WISDO VOICE · READY</span><label class="narration-toggle"><input type="checkbox" id="narration-mode"><i></i><span>🔊 NARRATION</span></label></div><div class="story-caption" id="story-caption"><small>WISDO NARRATOR</small><strong>Select a day, then play its story.</strong><p>The replay will travel entry by entry.</p></div><div class="wisdo-media-deck" id="wisdo-media-deck"><div class="media-orb"><i></i><span>MEDIA</span></div><div class="media-now"><small>WISDO SOUNDSTAGE</small><strong id="media-title">Cinema ambience ready</strong><span id="media-status">MUSIC · WISDO · AMBIENCE · SFX</span></div><button id="media-play" type="button" aria-label="Play or pause media">▶</button><label>MUSIC<input id="music-volume" type="range" min="0" max="100" value="28"></label><label>WISDO<input id="voice-volume" type="range" min="0" max="100" value="100"></label><label>AMBIENCE<input id="ambience-volume" type="range" min="0" max="100" value="18"></label><label>SFX<input id="sfx-volume" type="range" min="0" max="100" value="30"></label><form id="media-prompt"><input id="media-prompt-input" autocomplete="off" placeholder="Ask WISDO for music…"><button>✦</button></form></div>
        </section>
        <aside class="nexus-intel">
          <section class="holo-panel trade-stats" id="trade-stats"><small>SESSION PULSE</small><h3>Awaiting selection</h3><p>Select an active day in the month matrix.</p></section>
          <section class="holo-panel progress-panel" id="progress-panel"><small>DAILY TARGET</small><div class="progress-ring"><b>—</b></div><p>Target telemetry pending</p></section>
          <section class="holo-panel context-panel" id="context-panel"><small>WISDO AI CONTEXT</small><h3>Market memory</h3><p>Stored structure, session and signal intelligence will appear here.</p></section>
        </aside>
        <section class="nexus-trade holo-panel" id="trade-module"><small>SELECTED TRADE</small><h3>No trade selected</h3><p>Tap an ENTRY or CLOSE node in the replay.</p></section>
        <section class="nexus-core"><div class="core-beam"></div><div class="core-ring cr1"></div><div class="core-ring cr2"></div><div class="core-ring cr3"></div><div class="core-globe">W</div><strong>WISDO</strong><small>CALENDAR INTELLIGENCE CORE</small></section>
        <section class="nexus-reason holo-panel" id="why-module"><small>WHY THIS ENTRY?</small><h3>Decision memory</h3><p>Signal reasoning appears when a trade node is selected.</p></section>
      </main>
    ...
    const drawDay=date=>{const d=byDay[date]||{date,trades:[],pnl:0,wins:0,losses:0,buys:0,sells:0,lots:0},dayItems=itemsByDay[date]||[];document.querySelectorAll('.calendar-day').forEach(x=>x.classList.toggle('selected',x.dataset.date===date));const wr=d.trades.length?Math.round(d.wins/d.trades.length*100):0;document.querySelector('#trade-stats').innerHTML=`<small>TRADE STATS · ${html(date)}</small><h2 class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}</h2><span>DAY P/L</span><div class="stat-quad"><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win Rate</small></b><b>${d.lots.toFixed(2)}<small>Net Lots</small></b><b>${d.buys>d.sells?'BUY':d.sells>d.buys?'SELL':'MIX'}<small>Flow</small></b></div>`;document.querySelector('#theater-subtitle').textContent=new Date(date+'T12:00:00').toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
      ...
      const review=document.querySelector('#review-center'),market=document.querySelector('#mission-chart');if(review){market.style.display='none';review.hidden=false;const hours={};d.trades.forEach(t=>{const z=new Date(t.closed_at||t.updated_at||t.opened_at),h=Number.isNaN(z.getTime())?'Unknown':z.toLocaleTimeString([],{hour:'numeric'}),bucket=hours[h]||=( {pnl:0,trades:0,wins:0} );bucket.pnl+=Number(t.pnl||0);bucket.trades++;if(Number(t.pnl||0)>0)bucket.wins++;});const target=Number(localStorage.getItem('wisdo.hourlyTarget')||50),hourRows=Object.entries(hours).map(([h,v])=>{const per=v.trades?target/v.trades:0,perWinner=v.wins?target/v.wins:0;return `<div class="hour-row"><b>${html(h)}</b><span>ACTUAL <b class="${v.pnl>=0?'green':'red'}">${money(v.pnl)}</b></span><span>${v.trades} TRADES</span><span>${money(per)} / TRADE</span><span>${money(perWinner)} / WINNER</span><span>${v.pnl>=target?'CLOCK OUT ✓':'GAP '+money(target-v.pnl)}</span></div>`;}).join('')||'<p>No closed trades recorded for this date. Planning and notes are still available.</p>';const sortedReview=[...d.trades].sort((a,b)=>new Date(a.closed_at||a.updated_at||0)-new Date(b.closed_at||b.updated_at||0)),nodes=sortedReview.map(t=>`<div class="decision-node ${Number(t.pnl||0)<0?'loss':''}"><i></i><small>${new Date(t.closed_at||t.updated_at||t.opened_at).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</small><strong>${String(t.side||'').toUpperCase()} · ${money(t.pnl||0)}</strong></div>`).join(''),drops=dayItems.map(item=>`<div class="calendar-drop"><span class="creator-orb">${item.creatorLogo?`<img src="${html(item.creatorLogo)}" alt="">`:html(String(item.creatorName||'W').slice(0,1).toUpperCase())}</span><div><small>${html(String(item.audience||'private').toUpperCase())} · ${html(item.creatorName||'WISDO MEMBER')}</small><strong>${html(item.title||item.body||'Calendar note')}</strong></div><span>${item.type==='assignment'?'◷':item.audience==='global'?'🌐':'✦'}</span></div>`).join('')||'<p>No notes or Drops on this date yet.</p>';review.innerHTML=`<div class="review-head"><div><small>WISDO REVIEW CENTER // ${html(date)}</small><h2>${d.trades.length?'SESSION AUTOPSY':'FUTURE MISSION'}</h2><p>Learn from the session, simulate rules, and leave intelligence for your future self or community.</p></div><div class="review-actions"><button data-review-action="replay">REPLAY</button><button data-review-action="coach">ASK WISDO</button><button data-review-action="rule">BUILD RULE</button></div></div><div class="review-tabs"><button class="review-tab active">REVIEW</button><button class="review-tab">PAYCHECK</button><button class="review-tab">WHAT-IF</button><button class="review-tab">LESSONS</button><button class="review-tab">DROPS</button></div><div class="review-kpis"><b class="${d.pnl>=0?'green':'red'}">${money(d.pnl)}<small>Realized P/L</small></b><b>${d.trades.length}<small>Trades</small></b><b>${wr}%<small>Win rate</small></b><b>${Object.keys(hours).length}<small>Active hours</small></b><b>${money(target)}<small>Hourly target</small></b><b>${dayItems.length}<small>Notes / Drops</small></b></div><div class="review-grid"><section class="review-card hourly-lab"><small>HOURLY PAYCHECK ENGINE</small><h3>What did each hour need to produce?</h3><div class="hourly-controls"><label>Hourly wage $ <input id="hourly-target" type="number" min="0" step="1" value="${target}"></label><button class="review-tab" id="hourly-apply">RECALCULATE</button><span>Equal-trade and winner-funded requirements update from realized trades.</span></div><div>${hourRows}</div></section><section class="review-card"><small>DECISION SPINE</small><h3>Trade-by-trade memory</h3><div class="decision-spine">${nodes||'<span>No trade decisions recorded.</span>'}</div></section><section class="review-card"><small>WHAT-IF LAB</small><h3>Rule simulator</h3><div class="whatif-grid"><label>Hourly target<input id="sim-target" type="range" min="10" max="500" step="10" value="${Math.min(500,Math.max(10,target))}"></label><label>Max hourly loss<input id="sim-loss" type="range" min="10" max="500" step="10" value="50"></label><label>Max trades / hour<input id="sim-trades" type="range" min="1" max="30" value="10"></label><label>Cooldown minutes<input id="sim-cool" type="range" min="0" max="60" step="5" value="15"></label></div><div class="sim-result" id="sim-result">Simulation uses recorded realized trades only. It will not invent unobserved price paths.</div></section><section class="review-card"><small>CALENDAR MEMORY</small><h3>Private note or Calendar Drop</h3><form class="note-composer" id="calendar-note-form"><input id="note-title" placeholder="Title / future instruction"><textarea id="note-body" rows="3" placeholder="What should future-you or the community remember?"></textarea><select id="note-audience"><option value="private">🔒 Only me</option><option value="followers">⭐ Followers</option><option value="students">🎓 Students</option><option value="group">👥 Group</option><option value="global">🌐 Global — everyone</option></select><select id="note-type"><option value="note">Note</option><option value="lesson">Lesson</option><option value="goal">Goal</option><option value="event">Event</option><option value="assignment">Assignment</option><option value="drop">Calendar Drop</option></select><button type="submit">✦ SAVE TO ${html(date)}</button></form></section><section class="review-card"><small>LIVE CALENDAR LAYER</small><h3>Notes, assignments & Drops</h3><div class="calendar-drops">${drops}</div></section></div>`;document.querySelector('#hourly-apply')?.addEventListener('click',()=>{localStorage.setItem('wisdo.hourlyTarget',String(Number(document.querySelector('#hourly-target')?.value||50)));drawDay(date);});const simulate=()=>{const goal=Number(document.querySelector('#sim-target')?.value||target),maxLoss=Number(document.querySelector('#sim-loss')?.value||50),maxTrades=Number(document.querySelector('#sim-trades')?.value||10);let simulated=0,kept=0;for(const v of Object.values(hours)){let hp=0,count=0;const bucket=sortedReview.filter(t=>{const z=new Date(t.closed_at||t.updated_at||t.opened_at);return z.toLocaleTimeString([],{hour:'numeric'})===Object.keys(hours).find(k=>hours[k]===v);});for(const t of bucket){if(count>=maxTrades||hp>=goal||hp<=-maxLoss)break;hp+=Number(t.pnl||0);simulated+=Number(t.pnl||0);count++;kept++;}}document.querySelector('#sim-result').textContent=`RECORDED-TRADE SIMULATION · kept ${kept}/${d.trades.length} trades · simulated realized P/L ${money(simulated)} vs actual ${money(d.pnl)}. This does not model alternate market exits.`;};['sim-target','sim-loss','sim-trades','sim-cool'].forEach(id=>document.querySelector('#'+id)?.addEventListener('input',simulate));document.querySelector('#calendar-note-form')?.addEventListener('submit',async e=>{e.preventDefault();const payload={date,title:document.querySelector('#note-title').value,body:document.querySelector('#note-body').value,audience:document.querySelector('#note-audience').value,type:document.querySelector('#note-type').value};try{const created=await api('/api/v2/calendar/items',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});calendarItems.push(created.item);(itemsByDay[date]||=[]).push(created.item);drawMonth();drawDay(date);}catch(error){alert(error.message||'Could not save calendar item.');}});document.querySelector('[data-review-action="replay"]')?.addEventListener('click',()=>{review.hidden=true;market.style.display='';});document.querySelector('[data-review-action="coach"]')?.addEventListener('click',()=>window.dispatchEvent(new CustomEvent('wisdo:coach-request',{detail:{date,trades:d.trades,question:'Review this session with me.'}})));document.querySelector('[data-review-action="rule"]')?.addEventListener('click',()=>window.dispatchEvent(new CustomEvent('wisdo:rule-builder',{detail:{date,hourlyTarget:target}})));}
      const clock=t=>{const raw=t?.opened_at||t?.closed_at||t?.updated_at;if(!raw)return'time unknown';const z=new Date(raw);return Number.isNaN(z.getTime())?'time unknown':z.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'});},price=t=>Number(t?.open_price||0),close=t=>Number(t?.close_price||0),valid=sorted.filter(t=>price(t)&&close(t)),opens=valid.map(price),closes=valid.map(close),hi=Math.max(...opens,...closes,0),lo=Math.min(...opens.filter(Boolean),...closes.filter(Boolean)),startP=valid.length?price(valid[0]):0,endP=valid.length?close(valid.at(-1)):0,delta=startP&&endP?endP-startP:0,range=hi&&lo?hi-lo:0;
      let hh=0,hl=0,lh=0,ll=0,prevHi=null,prevLo=null;valid.forEach(t=>{const h=Math.max(price(t),close(t)),l=Math.min(price(t),close(t));if(prevHi!==null){if(h>prevHi)hh++;else lh++;}if(prevLo!==null){if(l>prevLo)hl++;else ll++;}prevHi=h;prevLo=l;});const structure=hh+hl>lh+ll?'higher-high / higher-low pressure':lh+ll>hh+hl?'lower-high / lower-low pressure':'a contested structure with no clean ruler',movement=delta>0?'climbed':delta<0?'descended':'returned near where it began',opening=first?String(first.side||'').toUpperCase():'market',closing=last?String(last.side||'').toUpperCase():'final',arc=d.wins>d.losses?'discipline found more light than resistance':d.losses>d.wins?'the market demanded tuition before surrendering clarity':'victory and resistance held each other in balance';
      const poem=`At ${clock(first)}, the first ${opening} entered near ${first?.open_price||'an uncaptured price'}. The clock had started, and price was already choosing its language.\n\nFrom ${startP||'the opening print'} to ${endP||'the final print'}, the recorded path ${movement}${delta?` by ${Math.abs(delta).toFixed(2)}`:''}. Its remembered range stretched roughly ${range?range.toFixed(2):'beyond captured resolution'}, while structure leaned into ${structure}. ${hh} higher highs, ${hl} higher lows, ${lh} lower highs and ${ll} lower lows formed the bones beneath the story.\n\nThen time began to matter. By ${clock(best)}, the strongest realized chapter carried ${money(Number(best?.pnl||0))}; around ${clock(worst)}, the deepest realized shadow answered with ${money(Number(worst?.pnl||0))}. Neither moment stood alone — each arrived after the price that came before it, each changed the meaning of what followed.\n\nAcross ${d.trades.length} trades and ${d.lots.toFixed(2)} lots, ${d.wins} positions found profit and ${d.losses} returned as lessons. ${arc}. Buyers appeared ${d.buys} times; sellers answered ${d.sells} times. The day was not one direction, but a sequence of pressure, acceptance, rejection and release.\n\nNear ${clock(last)}, the final ${closing} chapter closed the book. The session finished ${mood} at ${money(d.pnl)}. WISDO remembers the time, the price, and the structure together — where momentum expanded, where structure weakened, and where the next reading must become more precise.`;
      document.querySelector('#context-panel').innerHTML=`<small>WISDO AI // STRUCTURAL DAY POEM</small><h3 class="${d.pnl>=0?'green':'red'}">${d.pnl>=0?'Chronicle of Expansion':'Chronicle of Resistance'}</h3><div class="poem-structure"><span>⏱ ${clock(first)} → ${clock(last)}</span><span>◇ ${startP?startP.toFixed(2):'—'} → ${endP?endP.toFixed(2):'—'}</span><span>⌁ ${html(structure)}</span></div><div class="wisdo-day-poem">${poem.split('\n\n').map(x=>`<p>${html(x)}</p>`).join('')}</div><div class="poem-meta"><span>${d.trades.length} TRADES</span><span>${wr}% WIN RATE</span><span>RANGE ${range?range.toFixed(2):'—'}</span><span>${money(d.pnl)} CLOSE</span></div>`;
      const points=[];d.trades.forEach(t=>{const op=Number(t.open_price),cp=Number(t.close_price);if(Number.isFinite(op)&&op)points.push({kind:'entry',p:op,t:new Date(t.opened_at||t.updated_at).getTime(),trade:t});if(Number.isFinite(cp)&&cp)points.push({kind:'close',p:cp,t:new Date(t.closed_at||t.updated_at).getTime(),trade:t});});points.sort((a,b)=>a.t-b.t);let chart='<div class="chart-idle"><b>TRADE PATH NOT CAPTURED</b><span>Stored prices are insufficient for this day.</span></div>';
      if(points.length){const minT=Math.min(...points.map(q=>q.t)),maxT=Math.max(...points.map(q=>q.t)),minP=Math.min(...points.map(q=>q.p)),maxP=Math.max(...points.map(q=>q.p)),dx=Math.max(1,maxT-minT),dp=Math.max(.00001,maxP-minP),xy=q=>[38+(q.t-minT)/dx*924,272-(q.p-minP)/dp*220];let seg='',marks='';for(let i=1;i<points.length;i++){const A=xy(points[i-1]),B=xy(points[i]),up=points[i].p>=points[i-1].p;seg+=`<line class="trade-path ${up?'up':'down'}" x1="${A[0]}" y1="${A[1]}" x2="${B[0]}" y2="${B[1]}"/>`;}points.forEach((q,i)=>{const P=xy(q),t=q.trade,win=Number(t.pnl||0)>=0,side=String(t.side||'').toLowerCase().includes('sell')?'sell':'buy',tm=new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),lot=t.lots||t.volume||'—',ticket=t.ticket||t.order_id||t.id||'';marks+=`<g class="trade-marker" data-point="${i}" tabindex="0"><title>${html(q.kind.toUpperCase()+' · '+String(t.side||'').toUpperCase()+' · '+q.p+' · '+tm+' · '+lot+' lots · '+money(t.pnl||0))}</title><circle class="${q.kind==='entry'?'entry '+side:'close '+(win?'win':'loss')}" cx="${P[0]}" cy="${P[1]}" r="${q.kind==='entry'?8:9}"/><text class="marker-kind" x="${P[0]+11}" y="${P[1]-13}">${q.kind==='entry'?'ENTRY':'CLOSE'} ${i+1}</text><text class="marker-meta" x="${P[0]+11}" y="${P[1]+3}">${html(tm)} · ${html(String(q.p))}</text><text class="marker-meta marker-sub" x="${P[0]+11}" y="${P[1]+17}">${html(String(lot))} LOT${ticket?' · #'+html(String(ticket)):''}</text></g>`;});chart=`<div class="path-heat heat-green"></div><div class="path-heat heat-red"></div><svg viewBox="0 0 1000 310" preserveAspectRatio="none">${seg}${marks}</svg>`;}
      const mc=document.querySelector('#mission-chart');mc.classList.remove('idle');mc.innerHTML=chart+'<div class="chart-floor"></div><div class="chart-scan"></div><div class="chapter-stage" id="chapter-stage"></div>';const caption=document.querySelector('#story-caption'),narrationToggle=document.querySelector('#narration-mode');if(window.__wisdoStoryTimer){clearInterval(window.__wisdoStoryTimer);window.__wisdoStoryTimer=null;}if('speechSynthesis'in window)window.speechSynthesis.cancel();let storyTimer=null,storyIndex=0,spoken=null;const narrationKey='wisdo_calendar_narration';if(narrationToggle){narrationToggle.checked=localStorage.getItem(narrationKey)==='1';narrationToggle.onchange=()=>{localStorage.setItem(narrationKey,narrationToggle.checked?'1':'0');if(!narrationToggle.checked){if('speechSynthesis'in window)window.speechSynthesis.cancel();window.__wisdoNarrationAudio?.pause?.();}};}const memoryOf=t=>{const values=[t.entry_reason,t.signal_reason,t.reason,t.comment,t.magic_comment,t.strategy,t.bot_comment,t.decision_reason,t.setup,t.signal].filter(v=>String(v||'').trim()).map(v=>String(v).trim());return [...new Set(values)].join(' · ');};
      const tradeKey=t=>String(t.ticket||t.order_id||t.orderId||t.id||'');
      const campaignKey=t=>String(t.campaign_id||t.campaignId||t.strategy_id||t.magic||t.magic_number||t.symbol||'market');
      const groupCampaigns=()=>{const groups=[];let active=null;points.forEach((q,index)=>{const t=q.trade||{},key=campaignKey(t),time=Number(new Date(q.t));if(!active||active.key!==key||time-active.lastTime>45*60*1000){active={id:groups.length+1,key,points:[],lastTime:time};groups.push(active);}active.points.push({q,index});active.lastTime=time;});return groups;};
      const lessonFor=(q,index)=>{const prev=points[index-1],next=points[index+1],side=String(q.trade?.side||'').toLowerCase(),pnl=Number(q.trade?.pnl||0);if(!prev)return 'An opening position is a question, not an answer. Conviction still has to be earned.';const move=Number(q.p)-Number(prev.p),aligned=(side.includes('buy')&&move>0)||(side.includes('sell')&&move<0);if(q.kind==='entry'&&!aligned)return 'This add arrived against the latest recorded movement. Better price and broken thesis can wear the same coat; structure is what tells them apart.';if(q.kind==='entry'&&aligned)return 'The market is cooperating. That does not make the next add free. More exposure means more responsibility to the original idea.';if(q.kind==='close'&&pnl>0)return 'Profit is evidence of an outcome, not proof that every decision was sound. Separate the decision that created the edge from the ones that merely survived beside it.';if(q.kind==='close'&&pnl<0)return 'Red ink deserves a backward walk. The useful question is where price first contradicted the idea, not where the loss finally became visible.';if(next&&Math.abs(Number(next.p)-Number(q.p))>Math.max(.01,Math.abs(move))*1.5)return 'Expansion followed this location. Mark the place where price stopped negotiating and began to travel.';return 'One point is a fact. The sequence around it is the lesson.';};
      const sceneTitle=(q,index,localIndex,campaign)=>{const pnl=Number(q.trade?.pnl||0),prev=points[index-1],move=prev?Number(q.p)-Number(prev.p):0,side=String(q.trade?.side||'').toLowerCase(),aligned=(side.includes('buy')&&move>0)||(side.includes('sell')&&move<0);if(q.kind==='close')return pnl>=0?(localIndex%2?'The Receipt':'What the Road Returned'):(localIndex%2?'Tuition Paid':'Where the Thesis Broke');if(localIndex===0)return campaign.id===1?'The First Footstep':'A New Argument';if(aligned)return ['The Wind Answers','Pressing the Advantage','The Road Opens','Conviction Finds Company'][localIndex%4];return ['Weight Against the Thesis','The Dangerous Average','Price Refuses','An Argument Under Pressure'][localIndex%4];};
      const writeScene=(q,index,localIndex,campaign)=>{const t=q.trade||{},side=String(t.side||'').toUpperCase()||'POSITION',tm=new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),prev=points[index-1],move=prev?Number(q.p)-Number(prev.p):0,memory=memoryOf(t),pnl=Number(t.pnl||0),lesson=lessonFor(q,index),price=Number(q.p),title=sceneTitle(q,index,localIndex,campaign),priorEntry=campaign.points.slice(0,localIndex).reverse().find(x=>x.q.kind==='entry'),priorPrice=priorEntry?Number(priorEntry.q.p):null,relation=priorPrice===null?'':price>priorPrice?'above':price<priorPrice?'beneath':'back at',memoryLine=memory?` The machine had written this in its own hand: “${memory}.”`:' No private reasoning was preserved for this moment, so we will not invent one.';let narration,state,label,silence=700;if(q.kind==='entry'&&localIndex===0){narration=`${tm}. ${title}. ${side} entered at ${price.toFixed(2)}.${memoryLine} The first footprint is never certainty; it is a question placed in the path of price. ${lesson}`;state='OBSERVING';label='THESIS';silence=1200;}else if(q.kind==='entry'){const motion=move>0?'climbed':move<0?'descended':'returned',distance=Math.abs(move).toFixed(2);narration=localIndex%3===0?`Look here. By ${tm}, price had ${motion} ${distance} from the previous recorded event, and another ${side} joined ${relation} the earlier entry.${memoryLine} This is where size begins to reveal character. ${lesson}`:localIndex%3===1?`${title}. At ${tm}, the campaign added another ${side} at ${price.toFixed(2)}. Price stood ${relation} the earlier commitment.${memoryLine} Do not hurry past this moment. ${lesson}`:`There are moments when a campaign grows because the market agrees, and moments when it grows because the trader refuses to leave. At ${tm}, ${side} was added at ${price.toFixed(2)}.${memoryLine} ${lesson}`;state=move===0?'OBSERVING':'ATTACK';label='ADD';silence=900;}else if(pnl>=0){narration=`${title}. At ${tm}, this position left the field with ${money(pnl)}. It closed at ${price.toFixed(2)}.${memoryLine} Green is comforting, but comfort is a poor teacher. ${lesson}`;state='HARVEST';label='HARVEST';silence=1400;}else{narration=`${title}. At ${tm}, ${money(Math.abs(pnl))} became the price of this lesson. The position closed at ${price.toFixed(2)}.${memoryLine} We do not hide the wound, and we do not worship it. We study where the argument first lost the market. ${lesson}`;state='AUTOPSY';label='LESSON';silence=1700;}return{q,index,campaignId:campaign.id,title,narration,lesson,memory,state,label,silence,tradeKey:tradeKey(t),director:{focus:index,chapter:`CAMPAIGN ${campaign.id} · ${title}`,videoCue:q.kind==='close'?'campaign-memory':localIndex===0?'chapter-open':'none',truthLayer:'recorded-trade-data'}};};
      const buildDayManuscript=()=>{const campaigns=groupCampaigns(),scenes=[];campaigns.forEach(campaign=>campaign.points.forEach(({q,index},localIndex)=>scenes.push(writeScene(q,index,localIndex,campaign))));const pnl=Number(d.pnl||0),entries=points.filter(q=>q.kind==='entry').length;return{version:1,date:d.date||'',campaigns:campaigns.length,entries,pnl,scenes,epilogue:pnl>=0?`The day closes with ${money(pnl)} realized. ${entries} recorded entries crossed the page. Keep the profit, but carry forward the decisions that earned it; tomorrow will change the prices and test the lesson again.`:`The day closes at ${money(pnl)}. ${entries} recorded entries crossed the page. A losing day is not made wise by poetry. Its value comes from remembering exactly where price and reasoning separated, then refusing to pay twice for the same lesson.`};};
      let dayManuscript=buildDayManuscript(),campaignScript=dayManuscript.scenes,voiceState='READY',storyPaused=false;
      
      const setVoiceState=(state,detail='')=>{voiceState=state;mc.dataset.voiceState=state;const badge=document.querySelector('#wisdo-voice-state');if(badge){badge.textContent='WISDO VOICE · '+state+(detail?' · '+detail:'');badge.dataset.state=state;}};
      const speakBrowser=text=>new Promise(resolve=>{if(!('speechSynthesis'in window)||!text){resolve();return;}window.speechSynthesis.cancel();spoken=new SpeechSynthesisUtterance(text);spoken.rate=.78;spoken.pitch=.72;spoken.volume=1;const voices=window.speechSynthesis.getVoices(),voice=voices.find(v=>/en-US/i.test(v.lang)&&/david|daniel|aaron|alex|fred|male/i.test(v.name))||voices.find(v=>/en-US/i.test(v.lang))||voices[0];if(voice)spoken.voice=voice;let settled=false;const done=()=>{if(settled)return;settled=true;resolve();};spoken.onend=done;spoken.onerror=done;window.speechSynthesis.speak(spoken);});
      const voiceProbe=async()=>{try{const r=await fetch('/api/wisdo/voice/health',{credentials:'same-origin',cache:'no-store'}),j=await r.json().catch(()=>({}));return{http:r.status,...j};}catch(error){return{ok:false,code:'health_unreachable',detail:String(error?.message||error)}};};
      const voiceFailure=async(response,error)=>{let code='ENDPOINT',detail='';if(response){code='HTTP '+response.status;try{const body=await response.clone().json();code=body.code||code;detail=body.error||body.providerDetail||'';}catch{detail=await response.clone().text().catch(()=>'');}}else if(error){code=String(error?.message||'ENDPOINT');}const probe=await voiceProbe();window.__wisdoVoiceDiagnostic={at:new Date().toISOString(),code,detail:String(detail).slice(0,300),probe};console.warn('WISDO voice diagnostic',window.__wisdoVoiceDiagnostic);if(probe?.configured===false)code='NOT CONFIGURED';else if(probe?.ok===false&&probe?.http===404)code='ROUTE 404';setVoiceState('OFFLINE',String(code).slice(0,28).toUpperCase());};
      const speak=text=>new Promise(async resolve=>{if(!narrationToggle?.checked||!text){resolve();return;}setVoiceState('GENERATING');let response=null;try{response=await fetch('/api/wisdo/voice/speak',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'audio/mpeg'},body:JSON.stringify({text})});if(!response.ok){await voiceFailure(response);resolve();return;}if(!String(response.headers.get('content-type')||'').includes('audio')){await voiceFailure(response,new Error('NON AUDIO'));resolve();return;}const blob=await response.blob();if(!blob.size){await voiceFailure(null,new Error('EMPTY AUDIO'));resolve();return;}const url=URL.createObjectURL(blob),audio=new Audio(url);window.__wisdoNarrationAudio?.pause?.();window.__wisdoNarrationAudio=audio;const done=()=>{URL.revokeObjectURL(url);if(window.__wisdoNarrationAudio===audio)window.__wisdoNarrationAudio=null;duckMusic(false);setVoiceState('READY');resolve();};audio.onended=done;audio.onerror=async()=>{URL.revokeObjectURL(url);await voiceFailure(null,new Error('AUDIO PLAYBACK'));resolve();};audio.volume=Number(window.__wisdoAudioMix?.voice??1);duckMusic(true);setVoiceState('SPEAKING','WISDO');await audio.play();}catch(error){await voiceFailure(response,error);resolve();}});
            const narrate=async(q,index)=>{const t=q.trade,pnl=Number(t.pnl||0),side=String(t.side||'').toUpperCase(),price=q.kind==='entry'?t.open_price:t.close_price,node=mc.querySelector(`[data-point="${index}"]`);mc.querySelectorAll('.trade-marker').forEach(n=>n.classList.remove('story-focus'));node?.classList.add('story-focus');if(node){const circle=node.querySelector('circle'),svg=mc.querySelector('svg'),cx=Number(circle?.getAttribute('cx')||500),cy=Number(circle?.getAttribute('cy')||155),fx=(cx/1000*100).toFixed(2),fy=(cy/310*100).toFixed(2);svg?.style.setProperty('--focus-x',fx+'%');svg?.style.setProperty('--focus-y',fy+'%');svg?.classList.remove('story-riding');requestAnimationFrame(()=>requestAnimationFrame(()=>svg?.classList.add('story-riding')));mc.style.setProperty('--story-glow',pnl>=0?'rgba(54,255,179,.24)':'rgba(255,76,112,.22)');}const stage=mc.querySelector('#chapter-stage'),opened=new Date(t.opened_at||t.updated_at),closed=new Date(t.closed_at||t.updated_at),duration=Number.isNaN(opened.getTime())||Number.isNaN(closed.getTime())?'—':Math.max(0,Math.round((closed-opened)/60000))+'m',reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No stored entry reason',expression=pnl>0?'✦':pnl<0?'◇':'◉';if(stage){stage.classList.remove('chapter-in');stage.innerHTML=`<div class="wisdo-expression ${pnl>=0?'positive':'negative'}"><b>${expression}</b><span>WISDO</span></div><div class="chapter-readout"><small>CAMPAIGN EVENT ${index+1} / ${points.length}</small><strong>${html(side)} · ${q.kind.toUpperCase()}</strong><div><span>TIME <b>${html(new Date(q.t).toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'}))}</b></span><span>PRICE <b>${html(String(price||'—'))}</b></span><span>LOTS <b>${html(String(t.lots||t.volume||'—'))}</b></span><span>P/L <b>${money(pnl)}</b></span><span>HOLD <b>${html(duration)}</b></span></div><p>${html(q.kind==='entry'?reason:'Campaign result recorded at this exit.')}</p><em>OHLC candle replay requires Reporter candle history; WISDO will not invent missing candles.</em></div>`;requestAnimationFrame(()=>stage.classList.add('chapter-in'));}caption.innerHTML=`<small>CHAPTER ${index+1} / ${points.length} · ${q.kind.toUpperCase()}</small><strong>${q.kind==='entry'?`${side} entered at ${price||'—'}`:`${side} closed at ${price||'—'}`}</strong><p>${q.kind==='entry'?`WISDO follows the recorded trade event. This position would finish ${pnl>=0?'in profit':'in loss'} at ${money(pnl)}.`:`${pnl>=0?'The campaign realized profit here.':'The campaign realized a loss here.'} The review continues to the next recorded event.`}</p>`;const scene=campaignScript[index]||{narration:'WISDO has no recorded narration for this moment.',label:q.kind.toUpperCase(),state:'OBSERVING'};mc.dataset.storyState=scene.state;caption.querySelector('small').textContent=`CAMPAIGN ${scene.campaignId||1} · ${scene.title||scene.label}`;caption.querySelector('strong').textContent=scene.title||caption.querySelector('strong').textContent;caption.querySelector('p').textContent=scene.narration;mc.dataset.videoCue=scene.director?.videoCue||'none';await speak(scene.narration);node?.dispatchEvent(new Event('click'));};const stopStory=()=>{if(window.__wisdoStoryTimer){clearTimeout(window.__wisdoStoryTimer);window.__wisdoStoryTimer=null;}storyTimer=null;};const finishStory=()=>{stopStory();mc.querySelector('svg')?.classList.remove('story-riding');mc.classList.remove('story-live');caption.innerHTML+=`<em>DAY STORY COMPLETE · ${money(d.pnl)} · WISDO MEMORY STORED</em>`;speak(dayManuscript.epilogue);};const advanceStory=async()=>{if(storyPaused)return;if(storyIndex>=points.length){finishStory();return;}const current=storyIndex++;await narrate(points[current],current);if(storyIndex>=points.length){finishStory();return;}const pause=Number(campaignScript[current]?.silence||1100);window.__wisdoStoryTimer=setTimeout(advanceStory,pause);storyTimer=window.__wisdoStoryTimer;};const startStory=()=>{stopStory();storyIndex=0;storyPaused=false;dayManuscript=buildDayManuscript();campaignScript=dayManuscript.scenes;window.__wisdoDirectorScript=dayManuscript;if(!points.length)return;mc.classList.add('story-live');setVoiceState(narrationToggle?.checked?'READY':'MUTED');advanceStory();};window.__wisdoStoryTimer=setTimeout(startStory,700);storyTimer=window.__wisdoStoryTimer;
      mc.querySelectorAll('[data-point]').forEach(node=>node.onclick=()=>{const q=points[Number(node.dataset.point)],t=q.trade,pnl=Number(t.pnl||0),reason=t.entry_reason||t.signal_reason||t.reason||t.comment||t.strategy||'No entry reason was stored with this trade.';document.querySelector('#trade-module').innerHTML=`<small>SELECTED TRADE</small><h3 class="${pnl>=0?'green':'red'}">${pnl>=0?'WIN':'LOSS'} · ${html(String(t.side||'').toUpperCase())} · ${html(t.symbol||'')}</h3><div class="selected-grid"><b>Entry<small>${html(t.open_price||'—')}</small></b><b>Close<small>${html(t.close_price||'—')}</small></b><b>P/L<small>${money(pnl)}</small></b><b>Lots<small>${html(t.lots||t.volume||'—')}</small></b></div>`;document.querySelector('#why-module').innerHTML=`<small>WHY THIS ENTRY?</small><h3>${q.kind==='entry'?'Stored decision context':'Exit result'}</h3><p>${html(q.kind==='entry'?reason:`Closed at ${t.close_price||'—'} · ${money(pnl)}`)}</p>`;});};
