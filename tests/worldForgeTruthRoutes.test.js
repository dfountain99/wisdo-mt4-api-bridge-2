...
test('approved Aurelia Prime produces a pending truth report, then records Babylon observation',async()=>{
 ...
 try{
  ...
  const portal=operations.find(o=>o.type==='CREATE_PORTAL');
  assert.ok(portal,'golden world must contain a portal operation');
  const hidden={...visual,operations:operations.map(o=>o.id===portal.id?{...o,visible:false,inCamera:false}:o)};
  const rejected=await req('/api/world/personal/visual-report','POST',hidden);
  assert.equal(rejected.status,200);
  assert.ok(rejected.data.truthReport.requested.count>=8);
  assert.equal(rejected.data.truthReport.manifest.count,operations.length);
  assert.equal(rejected.data.truthReport.executed,operations.length);
  assert.equal(rejected.data.truthReport.visible,operations.length-1);
  assert.equal(rejected.data.truthReport.requirements.portal.status,'FAIL');
  assert.equal(rejected.data.truthReport.visual.operations.find(o=>o.id===portal.id).visible,false);
  assert.equal(rejected.data.truthReport.forgeStatus,'incomplete');
