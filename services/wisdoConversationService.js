...
export class WisdoConversationService {
  ...
  async behavior({input,text,intent,session,context}){
    ...
    if(!selected&&accounts.length>1)return {state:'clarification',text:`Which authorized account should this behavior control: ${accounts.map(accountLabel).join(', ')}?`};
    if(!selected)return {state:'unavailable',text:COACH_RESPONSES.unavailable};
    if(!connected(selected,this.staleMs))throw Object.assign(new Error('Trading connection offline.'),{code:'trading_offline'});
    const id=accountId(selected);this.safetyService.assertAccountAccess([id],accounts);if(compiled.mode!=='shadow')this.safetyService.assertVoiceExecutionMode([selected]);
    compiled={...compiled,scope:{...compiled.scope,level:compiled.scope.level||'account',account_id:id,account_ref:null}};
    ...
    const conflicts=await this.adaptiveFabricService.conflicts(input.userId,compiled);
    if(conflicts.some((conflict)=>conflict.severity==='blocking'))return {state:'clarification',conflicts,text:`That behavior conflicts with an active instruction: ${conflicts.map((conflict)=>conflict.reason).join(' ')} Pause, cancel, or revise the existing behavior first.`};
    const saved=await this.adaptiveFabricService.saveBehavior(input.userId,compiled,validation);
    if(compiled.mode==='shadow'){const shadow=await this.adaptiveFabricService.transitionBehavior(input.userId,saved.behavior_id,'shadow',input.userId);return {state:'shadow',behaviorId:saved.behavior_id,text:`Shadow simulation is active for ${compiled.name} on account ${accountLabel(selected)}. WISDO will evaluate the rule and explain what would happen, but it cannot send MT4 commands.`};}
    ...
    return {state:'awaiting_confirmation',behaviorId:saved.behavior_id,confirmationId:pending.confirmation_id,text:`I compiled “${compiled.name}” for account ${accountLabel(selected)}. Scope: ${compiled.scope.symbol||compiled.scope.bot_family||compiled.scope.magic_number||compiled.scope.campaign_id||'the selected account'}. Trigger: ${compiled.trigger.type.replaceAll('_',' ')}. Action: ${compiled.actions.map((action)=>action.type.replaceAll('_',' ')).join(', ')}. Exact scope, registered capabilities, and Reporter verification remain enforced. Say “Confirm Coach, execute” within 60 seconds to activate it.`};
  }

  async behaviorControl({input,intent,session}){
    if(!this.adaptiveFabricService)return {state:'unsupported',text:COACH_RESPONSES.unsupported};const behaviors=await this.adaptiveFabricService.listBehaviors(input.userId,{limit:50});
    if(intent.intent==='LIST_BEHAVIORS'){const visible=behaviors.filter((row)=>!['cancelled'].includes(row.status));return {state:'completed',behaviors:visible,text:visible.length?`Your behaviors are: ${visible.slice(0,10).map((row)=>`${row.name} (${row.status}, ID ${row.behavior_id})`).join('; ')}.`:'You have no active, paused, draft, or shadow trading behaviors.'};}
    const reference=String(intent.parameters?.reference||'').toLowerCase();let matches=reference?behaviors.filter((row)=>String(row.behavior_id).toLowerCase()===reference||String(row.name).toLowerCase().includes(reference)):behaviors.filter((row)=>['active','shadow','paused'].includes(row.status));
    if(matches.length!==1)return {state:'clarification',text:matches.length?'More than one behavior matched. Say the exact behavior ID.':'I could not find one matching behavior. Ask me to list your behaviors first.'};const behavior=matches[0];
    if(intent.intent==='PAUSE_BEHAVIOR'||intent.intent==='CANCEL_BEHAVIOR'){const target=intent.intent==='PAUSE_BEHAVIOR'?'paused':'cancelled';const updated=await this.adaptiveFabricService.transitionBehavior(input.userId,behavior.behavior_id,target,input.userId);return {state:target,behaviorId:behavior.behavior_id,text:`${updated.name} is ${target}. No new actions will be issued by it.`};}
    const pending=await this.confirmationService.create({userId:input.userId,sessionId:session.session_id,deviceId:input.deviceId,actionType:'RESUME_BEHAVIOR',accountIds:[behavior.scope?.account_id].filter(Boolean),parameters:{behaviorId:behavior.behavior_id},safetyLevel:'CONTROLLED'});return {state:'awaiting_confirmation',confirmationId:pending.confirmation_id,behaviorId:behavior.behavior_id,text:`WISDO is ready to resume ${behavior.name}. Say “Confirm Coach, execute” within 60 seconds.`};
  }
