...
export const ARCADE_BUILD='ARCADE-ALPHA2';
...
const GAME_ROWS = [
  ...
  ['liquidity-ghosts','Liquidity Ghosts','structure','liquidity','Learn stop hunts, news spikes, spread expansion, and FOMO through distinct ghost behaviors.','playable'],
  ['breakout-breaker','Breakout Breaker','structure','breakouts','Brick-breaker where support and resistance must be broken with valid confirmation.','playable'],
  ...
  ['risk-runner','Risk Runner','risk','risk','Endless runner where account survival depends on correct risk sizing.','playable'],
  ...
  ['fomo-frogger','FOMO Frogger','psychology','fomo','Cross candle traffic by waiting for confirmation instead of chasing.','playable'],
...
export const ARCADE_CATALOG=Object.freeze(GAME_ROWS.map(([id,name,floor,skill,description,status],index)=>Object.freeze({id,name,floor,skill,description,status,number:index+1,rewardMode:'skill_verified',wagering:false})));
...
function verifierFor(gameId){
  if(gameId==='bull-man')return{gameId,version:BULL_MAN_VERSION,tickMs:BULL_MAN_TICK_MS,maxTicks:BULL_MAN_MAX_TICKS,minTicks:20,lessons:BULL_MAN_LESSONS,replay:(seed,inputs)=>replayBullMan(seed,inputs)};
  const def=getAlpha2GameDef(gameId);if(!def)return null;
  return{gameId,version:def.version,tickMs:def.tickMs,maxTicks:def.maxTicks,minTicks:def.minTicks,lessons:def.lessons,replay:(seed,inputs)=>replayAlpha2Game(gameId,seed,inputs),controls:def.controls,objective:def.objective};
}

export function scoreArcadeEducation(gameId,result={},answers=[]){
  const key=ANSWER_KEYS[gameId];if(!key)throw new Error('No education verifier exists for this arcade game.');
  ...
  let riskDiscipline=Math.max(0,Math.min(100,100-hits*18-mistakes*9));
  if(Number.isFinite(Number(result.equity)))riskDiscipline=Math.round(Math.max(0,Math.min(100,Number(result.equity))));
  if(Number(result.lives||0)>=3)riskDiscipline=Math.min(100,riskDiscipline+5);
...
export function arcadeEconomyPolicy(env=process.env){
  ...
  return Object.freeze({currency:'CULTURE_COIN',wagering:false,serverAuthoritative:true,dailyEarnCap:integer(env.WISDO_ARCADE_DAILY_COIN_CAP,1,100000,60),maxCoinsPerSession:integer(env.WISDO_ARCADE_MAX_COIN_PER_SESSION,1,1000,15),redemptionEnabled:asBool(env.WISDO_CULTURE_COIN_REDEMPTION_ENABLED),usdCentsPerCoin:Number.isFinite(centsRaw)&&centsRaw>0?integer(centsRaw,1,100000,null):null,notice:'Culture Coin arcade rewards are skill-earned and never wagered. Cash/redemption value is available only when an authorized redemption policy is explicitly enabled.'});
}

export function computeArcadeReward(gameId,result,answers,policy=arcadeEconomyPolicy({})){
  const education=scoreArcadeEducation(gameId,result,answers);const verifier=verifierFor(gameId);const eligible=Number(result?.ticks||0)>=Number(verifier?.minTicks||20)&&education.weighted>=35;
  let coins=eligible?Math.floor(education.weighted/10):0;if(result?.status==='won')coins+=3;coins+=Math.floor(Math.max(0,Math.min(1,Number(result?.completion||0)))*3);coins=Math.max(0,Math.min(Number(policy.maxCoinsPerSession||15),coins));return Object.freeze({eligible,coins,education});
}
...
export class WisdoArcadeService{
  ...
  policy(){return arcadeEconomyPolicy(this.env);} catalog(){return ARCADE_CATALOG;}
  async ensureSchema(){
  ...
  async startSession(userId,gameId='bull-man'){
    await this.ensureSchema();const game=gameById(gameId);if(!game){const error=new Error('Arcade game not found.');error.statusCode=404;throw error;}if(game.status!=='playable'){const error=new Error(`${game.name} is in the 50-game roadmap but is not playable in this build yet.`);error.statusCode=409;throw error;}
    const verifier=verifierFor(game.id);if(!verifier){const error=new Error('This game does not have a server verifier.');error.statusCode=409;throw error;}
    const sessionId=crypto.randomUUID(),seed=crypto.randomInt(1,2_000_000_000),expiresAt=new Date(Date.now()+30*60*1000);
    ...
    return {...result.rows[0],build:ARCADE_BUILD,tickMs:verifier.tickMs,maxTicks:verifier.maxTicks,minTicks:verifier.minTicks,lessons:verifier.lessons,controls:verifier.controls||'MOVE',objective:verifier.objective||game.description,policy:this.policy()};
  }
  async finishSession(userId,sessionId,payload={}){
    ...
    try{
      ...
      const verifier=verifierFor(row.game_id);if(!verifier){const error=new Error('This game does not have a verifier in the current build.');error.statusCode=409;throw error;}
      const inputs=Array.isArray(payload.inputs)?payload.inputs.slice(0,verifier.maxTicks).map((x)=>clean(x,10).toLowerCase()):[];const answers=Array.isArray(payload.quizAnswers)?payload.quizAnswers.slice(0,verifier.lessons.length).map((x)=>integer(x,-1,10,-1)):[];
      if(inputs.length<verifier.minTicks){const error=new Error('Not enough verified gameplay ticks to finalize this session.');error.statusCode=400;throw error;}
      const elapsedMs=Date.now()-new Date(row.started_at).getTime();const perTickFloor=Math.max(24,Math.min(70,Number(verifier.tickMs||100)*.42));const minimumElapsedMs=Math.min(18000,Math.max(2000,Math.round(inputs.length*perTickFloor)));
      ...
      const daily=await client.query(`SELECT COALESCE(SUM(amount),0)::int AS earned FROM wisdo_culture_coin_ledger WHERE user_id=$1 AND source_type='arcade' AND amount>0 AND created_at>=date_trunc('day',NOW())`,[clean(userId,200)]);const already=Number(daily.rows[0]?.earned||0),remaining=Math.max(0,policy.dailyEarnCap-already),award=Math.min(reward.coins,remaining),digest=crypto.createHash('sha256').update(JSON.stringify(inputs)).digest('hex');
  ...
  async profile(userId){
    await this.ensureSchema();const [ledger,sessions]=await Promise.all([
      ...
      this.pool.query(`SELECT COUNT(*)::int AS sessions,COUNT(*) FILTER(WHERE status='finalized')::int AS finalized,COALESCE(MAX((result->'verified'->>'score')::int),0)::int AS best_score,COALESCE(MAX((result->'verified'->>'score')::int) FILTER(WHERE game_id='bull-man'),0)::int AS best_bull_man_score FROM wisdo_arcade_sessions WHERE user_id=$1`,[clean(userId,200)]),
    ]);
    return{cultureCoinBalance:Number(ledger.rows[0]?.balance||0),earnedToday:Number(ledger.rows[0]?.earned_today||0),sessions:Number(sessions.rows[0]?.sessions||0),finalized:Number(sessions.rows[0]?.finalized||0),bestArcadeScore:Number(sessions.rows[0]?.best_score||0),bestBullManScore:Number(sessions.rows[0]?.best_bull_man_score||0),policy:this.policy()};
  }
  async leaderboard(gameId='bull-man',limit=20){await this.ensureSchema();const game=gameById(gameId);if(!game){const error=new Error('Arcade game not found.');error.statusCode=404;throw error;}const result=await this.pool.query(`SELECT user_id,MAX((result->'verified'->>'score')::int)::int AS score,MAX((result->'education'->>'weighted')::int)::int AS mastery,COUNT(*)::int AS sessions FROM wisdo_arcade_sessions WHERE game_id=$1 AND status='finalized' GROUP BY user_id ORDER BY score DESC,mastery DESC LIMIT $2`,[game.id,integer(limit,1,100,20)]);return result.rows.map((row)=>({player:`OP-${crypto.createHash('sha256').update(String(row.user_id)).digest('hex').slice(0,8).toUpperCase()}`,score:Number(row.score||0),mastery:Number(row.mastery||0),sessions:Number(row.sessions||0)}));}
  async health(){await this.ensureSchema();const result=await this.pool.query(`SELECT COUNT(*)::int AS sessions FROM wisdo_arcade_sessions`);return{ok:true,service:'wisdo-arcade',build:ARCADE_BUILD,alpha2CoreVersion:ALPHA2_VERSION,catalogGames:ARCADE_CATALOG.length,playableGames:ARCADE_CATALOG.filter((g)=>g.status==='playable').length,playableIds:ARCADE_CATALOG.filter((g)=>g.status==='playable').map((g)=>g.id),sessions:Number(result.rows[0]?.sessions||0),policy:this.policy()};}
}
