...
function getReporterDelivery() {
  ...
  if (existsSync(packagePath) && existsSync(compiledPath) && existsSync(sourcePath)) {
    ...
    notes.push(
      'Reporter source is attached below: `CultureCoin_MT4_Reporter.mq4`. Open it in MetaEditor and compile it to create the `.ex4` file before placing it in `MQL4 -> Experts`.',
    );
  } else {
...
function buildNoSnapshotEmbed(status, mt4SyncService) {
  ...
  const setupSteps = [
    ...
    '2. Install CultureCoin_MT4_Reporter.ex4 into MQL4 -> Experts.',
    '3. Attach the Reporter EA to any chart.',
