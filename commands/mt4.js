...
function getReporterDelivery() {
  ...
  if (existsSync(packagePath) && existsSync(compiledPath) && existsSync(sourcePath)) {
    ...
    notes.push(
      'Reporter v1.57 source is attached below: `CultureCoin_MT4_Reporter.mq4`. Compile it in MetaEditor, replace the older Reporter on every follower terminal, and confirm the chart dashboard says v1.57 before testing lead-close relay.',
    );
  } else {
...
function buildNoSnapshotEmbed(status, mt4SyncService) {
  ...
  const setupSteps = [
    ...
    '2. Compile the attached CultureCoin_MT4_Reporter.mq4 in MetaEditor and install the resulting v1.57 EX4 into MQL4 -> Experts.',
    '3. Remove the older Reporter, then attach the newly compiled Reporter v1.57 to any chart.',
