...
export async function startApiServer({ config, mt4SyncService, mt4CommandService, copyTradingService, tradeSignalService, deskDashboardService, rankService, announcementService, paymentService, logger, client = null, operatorDeskService = null, accountSelectionService = null, commandRegistryAudit = null, signalGridService: providedSignalGridService = null, signalCopyService: providedSignalCopyService = null, discordSignalGridService: providedDiscordSignalGridService = null }) {
  ...
  // WISDO Render Node: self-hosted ComfyUI. Generated bytes stay off this server heap.
  const comfyBase=()=>String(process.env.WISDO_COMFYUI_URL||'').trim().replace(/\/$/,'');
  const comfyTemplate=()=>{try{return JSON.parse(String(process.env.WISDO_COMFYUI_WORKFLOW_JSON||''));}catch{return null;}};
  const deepReplace=(v,r)=>typeof v==='string'?Object.entries(r).reduce((x,[k,y])=>x.split(k).join(String(y)),v):Array.isArray(v)?v.map(x=>deepReplace(x,r)):(v&&typeof v==='object'?Object.fromEntries(Object.entries(v).map(([k,x])=>[k,deepReplace(x,r)])):v);
  async function comfyFetch(pathname,options={}){if(!comfyBase())throw Object.assign(new Error('WISDO_COMFYUI_URL is not configured.'),{statusCode:503,code:'WISDO_COMFYUI_NOT_CONFIGURED'});const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),Number(process.env.WISDO_COMFYUI_TIMEOUT_MS||20000));try{return await fetch(comfyBase()+pathname,{...options,signal:controller.signal});}finally{clearTimeout(timer);}}

  app.post('/api/wisdo/media/video',async(req,res)=>{
    const prompt=String(req.body?.prompt||'').trim().slice(0,6000),template=comfyTemplate();
    if(!prompt)return res.status(400).json({ok:false,code:'WISDO_VIDEO_PROMPT_REQUIRED',error:'A video prompt is required.'});
    if(!comfyBase())return res.status(503).json({ok:false,code:'WISDO_COMFYUI_NOT_CONFIGURED',error:'Set WISDO_COMFYUI_URL to the private ComfyUI Render Node.'});
    if(!template)return res.status(503).json({ok:false,code:'WISDO_COMFYUI_WORKFLOW_REQUIRED',error:'Set WISDO_COMFYUI_WORKFLOW_JSON to an API-format Wan workflow template.'});
    const seed=Number.isFinite(Number(req.body?.seed))?Math.floor(Number(req.body.seed)):Math.floor(Math.random()*2147483647);
    const workflow=deepReplace(template,{'{{PROMPT}}':prompt,'{{NEGATIVE_PROMPT}}':String(req.body?.negativePrompt||'low quality, blurry, distorted, text, watermark'),'{{SEED}}':seed,'{{WIDTH}}':Math.max(256,Math.min(1920,Number(req.body?.width)||832)),'{{HEIGHT}}':Math.max(256,Math.min(1920,Number(req.body?.height)||480)),'{{FRAMES}}':Math.max(17,Math.min(241,Number(req.body?.frames)||81))});
    try{const u=await comfyFetch('/prompt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:workflow,client_id:'wisdo-render-node'})}),p=await u.json().catch(()=>({}));if(!u.ok||!p.prompt_id)return res.status(502).json({ok:false,code:'WISDO_COMFYUI_QUEUE_FAILED',error:'ComfyUI rejected the workflow.',detail:p});return res.status(202).json({ok:true,jobId:p.prompt_id,status:'queued',provider:'comfyui-local',seed,statusUrl:`/api/wisdo/media/video/${encodeURIComponent(p.prompt_id)}`,message:'Video queued on the private WISDO Render Node.'});}catch(e){return res.status(e?.statusCode||502).json({ok:false,code:e?.code||'WISDO_COMFYUI_FAILED',error:e?.name==='AbortError'?'ComfyUI Render Node timed out.':e.message});}
  });
  app.get('/api/wisdo/media/video/:jobId',async(req,res)=>{const id=String(req.params.jobId||'').replace(/[^a-zA-Z0-9_-]/g,'');try{const u=await comfyFetch('/history/'+encodeURIComponent(id)),h=await u.json().catch(()=>({})),job=h[id];if(!job)return res.json({ok:true,jobId:id,status:'queued',provider:'comfyui-local'});const outputs=[];for(const node of Object.values(job.outputs||{}))for(const file of [...(node?.videos||[]),...(node?.gifs||[]),...(node?.images||[])])if(file?.filename)outputs.push({filename:file.filename,url:`/api/wisdo/media/video/${encodeURIComponent(id)}/output?filename=${encodeURIComponent(file.filename)}&subfolder=${encodeURIComponent(file.subfolder||'')}&type=${encodeURIComponent(file.type||'output')}`});return res.json({ok:true,jobId:id,status:outputs.length?'ready':(job.status?.status_str==='error'?'failed':'running'),provider:'comfyui-local',outputs,resultUrl:outputs[0]?.url||null,completed:Boolean(outputs.length)});}catch(e){return res.status(e?.statusCode||502).json({ok:false,code:e?.code||'WISDO_COMFYUI_STATUS_FAILED',error:e.message});}});
  app.get('/api/wisdo/media/video/:jobId/output',async(req,res)=>{const filename=String(req.query.filename||'').replace(/[\\/]/g,''),subfolder=String(req.query.subfolder||'').replace(/\.\.|[\\]/g,'').replace(/^\/+|\/+$/g,''),type=['input','temp','output'].includes(String(req.query.type))?String(req.query.type):'output';if(!filename)return res.status(400).json({ok:false,error:'Output filename required.'});try{const u=await comfyFetch('/view?'+new URLSearchParams({filename,subfolder,type}));if(!u.ok)return res.status(502).json({ok:false,error:'ComfyUI output unavailable.'});res.setHeader('Content-Type',u.headers.get('content-type')||'video/mp4');res.setHeader('Cache-Control','private, max-age=3600');const bytes=Buffer.from(await u.arrayBuffer());return res.send(bytes);}catch(e){return res.status(e?.statusCode||502).json({ok:false,code:e?.code||'WISDO_COMFYUI_OUTPUT_FAILED',error:e.message});}});
  ...
  registerPresenceIdentityRoutes(app, {
    loadEcosystemState,
    saveEcosystemState,
    paymentService,
    rankService,
