...
import { attachCloseTrackerCommand, buildTrendAnalytics, commandForCloseMode, createCloseTracker, ensureCloseIntelligenceState, listCloseTrackers } from '../services/tradeCloseIntelligence.js';
import {
...
async function mutate(load,save,fn){
  const operation = ecosystemMutationQueue.then(async()=>{ const state=ensureMajorState(await load()); const result=await fn(state); if(result?.[MUTATION_CONTROL]){ if(result.save) await save(state); return result.value; } await save(state); return result; });
...
export function registerMajorUpgradeRoutes(app,{config,loadEcosystemState,saveEcosystemState,mt4SyncService,mt4CommandService,copyTradingService,logger}){
  const queueBulkClose=async(req,res,forcedMode='')=>{
    ...
    try{
      tracker=await mutate(loadEcosystemState,saveEcosystemState,working=>{
        const row=createCloseTracker(working,{userId:req.wisdoUser.id,accountId,mode:mode==='profitable'?'profitable':mode==='losing'?'losing':'all',requestSource:'website'});
        appendMemberAlert(working,req.wisdoUser.id,{type:'system',title:`${row.label} requested`,body:'The command entered the immediate MT4 priority queue. WISDO will attach completion results and Compound Tracker analysis when Reporter confirms execution.',metadata:{accountId,trackerId:row.id,mode:row.mode}},`close-requested:${row.id}`);
        audit(working,req.wisdoUser.id,'trade.bulk_close_requested','TradingAccount',accountId,{mode:row.mode,trackerId:row.id,command:commandName});
        return row;
      });
      const command=await mt4CommandService.queueCommandForAccount(req.wisdoUser.id,accountId,commandName,{accountId,confirmation:req.body?.confirmation,immediate:true,priority:1000,ttlMinutes:10,closeMode:tracker.mode,compoundTrackerId:tracker.id,requestedFrom:'website_trade_control'});
      tracker=await mutate(loadEcosystemState,saveEcosystemState,working=>attachCloseTrackerCommand(working,tracker.id,command.id));
      return res.json({ok:true,queued:true,immediate:true,mode:tracker.mode,command,tracker});
    }catch(error){
      if(tracker?.id){
        await mutate(loadEcosystemState,saveEcosystemState,working=>{const row=working.compoundCloseTrackersById[tracker.id];if(row){row.status='queue_failed';row.result={success:false,message:error.message};row.updated_at=nowIso();}return row||null;}).catch(()=>undefined);
      }
  ...
  app.get('/api/v2/analyzer/trends',requireUser,async(req,res)=>{await synchronizeLiveTradeLedger({userId:req.wisdoUser.id,mt4SyncService,loadEcosystemState,saveEcosystemState});const state=ensureMajorState(await loadEcosystemState());res.json({ok:true,...buildTrendAnalytics(state,req.wisdoUser.id,req.query.account_id||'')})});
  ...
  app.get('/api/v2/culture-lanes',requireUser,async(req,res)=>{
    const state=ensureMajorState(await loadEcosystemState());
    const lanes=Object.values(state.cultureLanesById).filter(lane=>laneAccessibleTo(lane,req.wisdoUser.id));
    res.json({ok:true,lanes,source:'culture-lane-os-v6'});
  });
  ...
  app.put('/api/v2/culture-lanes/:laneId/symbol-policy',requireUser,async(req,res)=>{
    const policy=await mutate(loadEcosystemState,saveEcosystemState,state=>setLaneSymbolPolicy(state,req.params.laneId,req.wisdoUser.id,req.body||{}));
  ...
  app.post('/api/v2/culture-lanes/:laneId/harvest/evaluate',requireUser,async(req,res)=>{
    ...
    for(const account of evaluation.vault.accounts){
      try{
        const command=await mt4CommandService.queueCommandForAccount(req.wisdoUser.id,account.accountId,'CLOSE_ALL_TRADES',{accountId:account.accountId,confirmation:'confirmed',immediate:true,priority:2000,ttlMinutes:2,requestedFrom:'culture_lane_harvest',laneId:req.params.laneId});
        commandIds.push(command.id);
      }catch(error){failures.push({accountId:account.accountId,error:error.message});}
    }
    const cycle=await mutate(loadEcosystemState,saveEcosystemState,working=>{
      ...
      created.failures=failures; created.status=failures.length?'partially_queued':'commands_queued'; created.updatedAt=nowIso();
      return created;
    });
  ...
  logger?.info?.('WISDO major upgrade routes registered', { source: 'wisdo-culture-lane-os-v6.0.0.zip', version: '6.0.0' });
}
