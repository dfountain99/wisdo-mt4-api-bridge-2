...
function authPage(mode='login',message='',options={}){
 ...
 const fields=isRegister?`${returnField}<label>Full name<input class="input" name="name" required></label><label>Email<input class="input" type="email" name="email" required></label><label>Password<input class="input" type="password" name="password" minlength="8" required></label>`:forgot?`<label>Email<input class="input" type="email" name="email" required></label>`:reset?`<input type="hidden" name="token" value="${esc(resetToken)}"><label>New password<input class="input" type="password" name="password" minlength="8" required></label>`:`${returnField}<label>Email<input class="input" type="email" name="email" required></label><label>Password<input class="input" type="password" name="password" required></label>`;
...
export function registerMajorUpgradeRoutes(app,{config,loadEcosystemState,saveEcosystemState,mt4SyncService,mt4CommandService,copyTradingService,logger}){
  ...
  app.get('/copier',(req,res)=>res.send(productPage('copier'))); app.get('/analyzer',(req,res)=>res.send(productPage('analyzer'))); app.get('/compare',(req,res)=>res.send(productPage('compare'))); app.get('/pricing',(req,res)=>res.send(pricingPage())); app.get('/academy',(req,res)=>res.send(academyPage())); app.get('/blog',(req,res)=>res.send(blogPage())); app.get('/resources',(req,res)=>res.send(publicShell({title:'WISDO Resource Center',description:'Original trading guides, checklists, journals, calculators, seminars, and adaptive learning resources.',path:'/resources',active:'/resources',body:`<main><section class="page-hero"><span class="eyebrow">Resource Center</span><h1>Study guides, tools, and live learning in one library.</h1><p class="lead">Members receive original WISDO checklists, worksheets, journals, flash cards, calculators, seminars, interactive labs, bookmarks, notes, and adaptive AI teaching without redistributing copyrighted paid courses.</p><div class="actions"><a class="btn primary" href="/app/education">Open member library</a><a class="btn ghost" href="/academy">Explore Academy</a></div></section><section class="page-section"><div class="grid4">${[['Trading Academy','Beginner through professional market education.'],['WISDO University','Copier, account health, automation, DF Sauce, and HIGHTOWER operating knowledge.'],['Resource Center','Original guides, journals, checklists, cheat sheets, and calculators.'],['Live Learning','Webinars, seminars, market breakdowns, office hours, and community Q&A.']].map(([title,copy])=>`<article class="card"><h3>${title}</h3><p>${copy}</p></article>`).join('')}</div></section></main>`}))); app.get('/blog/:slug',(req,res)=>{const p=BLOG_POSTS.find(x=>x.slug===req.params.slug);if(!p)return res.status(404).send(publicShell({title:'Not found',description:'Resource not found.',path:req.path,body:'<main class="page-hero"><h1>Resource not found.</h1></main>'}));res.send(blogPostPage(p));});
  ...
  app.get('/register',(req,res)=>res.send(authPage('register','',{returnTo:req.query.returnTo}))); app.get('/signup',(req,res)=>res.send(authPage('register','',{returnTo:req.query.returnTo}))); app.get('/login',(req,res)=>res.send(authPage('login',String(req.query.message||''),{returnTo:req.query.returnTo}))); app.get('/forgot-password',(req,res)=>res.send(authPage('forgot'))); app.get('/reset-password',(req,res)=>res.send(authPage('reset','',{token:req.query.token})));
  ...
  app.get('/api/v2/copier-rules',requireUser,async(req,res)=>{const relaySync=await synchronizeCopierRulesToRelay({userId:req.wisdoUser.id,mt4SyncService,loadEcosystemState});const state=ensureMajorState(await loadEcosystemState());res.json({ok:true,rules:Object.values(state.copierRules).filter(r=>String(r.user_id)===String(req.wisdoUser.id)),relaySync})});
  app.get('/api/v2/copier/diagnostics',requireUser,async(req,res)=>{
    const userId=String(req.wisdoUser.id);
    const relaySync=await synchronizeCopierRulesToRelay({userId,mt4SyncService,loadEcosystemState});
    const state=ensureMajorState(await loadEcosystemState());
  ...
  app.post('/api/v2/copier-rules',requireUser,async(req,res)=>{
    const outcome=await mutate(loadEcosystemState,saveEcosystemState,state=>{
      ...
      const risk=RISK_TYPES.includes(req.body.risk_type)?req.body.risk_type:'multiplier';const rule={id:id('rule'),user_id:String(req.wisdoUser.id),master_id:String(master.id),slave_id:String(follower.id),risk_type:risk,risk_value:num(req.body.risk_value,1),min_lot:num(req.body.min_lot,.01),max_lot:num(req.body.max_lot,100),equity_protection_pct:req.body.equity_protection_pct===''?null:num(req.body.equity_protection_pct,null),max_daily_loss:req.body.max_daily_loss===''?null:num(req.body.max_daily_loss,null),max_open_trades:req.body.max_open_trades===''?null:num(req.body.max_open_trades,null),max_spread_points:req.body.max_spread_points===''?null:num(req.body.max_spread_points,null),max_slippage_points:req.body.max_slippage_points===''?null:num(req.body.max_slippage_points,null),allowed_symbols:parseSymbols(req.body.allowed_symbols),symbol_mapping:normalizeMap(req.body.symbol_mapping),trading_hours_start:req.body.trading_hours_start||null,trading_hours_end:req.body.trading_hours_end||null,is_active:req.body.is_active===undefined?true:parseBool(req.body.is_active),reverse_signals:parseBool(req.body.reverse_signals),copy_sl_tp:req.body.copy_sl_tp===undefined?true:parseBool(req.body.copy_sl_tp),copy_pending_orders:parseBool(req.body.copy_pending_orders),created_at:nowIso(),updated_at:nowIso()};state.copierRules[rule.id]=rule;audit(state,req.wisdoUser.id,'copier_rule.created','CopierRule',rule.id,{leadRole:master.desk_role,receiverRole:follower.desk_role});return {rule};
    });
    if(outcome.error)return res.status(400).json({ok:false,error:outcome.error});const relayRoute=await syncCopierRuleToRelay(mt4SyncService,outcome.rule);await mutate(loadEcosystemState,saveEcosystemState,state=>{appendMemberAlert(state,req.wisdoUser.id,{type:relayRoute?'system':'system',title:relayRoute?'Culture Lane execution armed':'Culture Lane saved — Reporter relay pending',body:relayRoute?`${outcome.rule.master_id} → ${outcome.rule.slave_id} is registered in the live relay engine.`:'The lane is saved, but one or both accounts must complete Reporter synchronization before live copying can start.',metadata:{ruleId:outcome.rule.id,relayRegistered:Boolean(relayRoute)}},`lane-created:${outcome.rule.id}`);return true});res.status(201).json({ok:true,rule:outcome.rule,relayRoute,executionReady:Boolean(relayRoute)});
  });
  ...
  app.get('/api/public/health',async(req,res)=>{const security=sessionSecurityStatus();res.json({ok:true,service:'WISDO Major Product Pass',version:'5.3.0',time:nowIso(),persistence:config.persistence?.mode||'json',security,integrations:{discord:Boolean(process.env.DISCORD_TOKEN&&process.env.CLIENT_ID),stripe:Boolean(process.env.STRIPE_SECRET_KEY),resend:Boolean(process.env.RESEND_API_KEY),market:Boolean(process.env.FINNHUB_API_KEY||process.env.TRADING_ECONOMICS_API_KEY||process.env.FIRECRAWL_API_KEY),ai:Boolean(process.env.OPENAI_API_KEY||process.env.GOOGLE_AI_API_KEY),postgres:Boolean(process.env.DATABASE_URL)},features:{premiumPublicSite:true,pricingConfigurator:true,operationalApi:true,signedBrokerWebhook:true,accountSpecificCommands:true,academy:true,affiliate:true,unifiedCopierOptions:true,protectedPrivateStrategies:true}})});
  app.get('/api/runtime-audit',async(req,res)=>{const state=ensureMajorState(await loadEcosystemState());res.json({ok:true,version:'5.3.0',source:'wisdo-member-app-product-pass(1).zip',checks:{rootRoute:true,publicProductPages:true,loginReturnTo:true,signedSessions:sessionSecurityStatus().signedSessions,credentialEncryptionReady:sessionSecurityStatus().credentialEncryptionConfigured,copierRules:Object.keys(state.copierRules).length,accounts:Object.keys(state.tradingAccounts).length,trades:Object.keys(state.trades).length,closeSignalsBypassEntryFilters:true,followerSymbolGuaranteed:true,persistentStorageConfigured:Boolean(config.persistence?.storagePath),executionAutomatchEnabled:parseBool(process.env.WISDO_SYMBOL_AUTOMATCH_EXECUTION_ENABLED),unifiedCopierOptions:true,privateStrategySourcePublic:false}})});
  ...
  logger?.info?.('WISDO major upgrade routes registered', { source: 'wisdo-member-app-product-pass(1).zip', version: '5.3.0' });
}
