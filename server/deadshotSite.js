...
function tcDashboardVisual() {
  return `<div class="tc-dash-wrap"><div class="tc-dashboard"><div class="tc-dashboard-shell"><div class="tc-side"><div class="active">Dashboard</div><div>Copier Engine</div><div>Accounts</div><div>Trades</div><div>Performance</div><div>Seminar</div></div><div><div class="tc-mini-grid"><div class="tc-mini"><span>Illustrative Portfolio</span><strong>Example</strong></div><div class="tc-mini"><span>Master Lane</span><strong>Example</strong></div><div class="tc-mini"><span>Follower Lanes</span><strong>Example</strong></div><div class="tc-mini"><span>Open Positions</span><strong>Example</strong></div></div><div class="tc-table-card"><h3>Copier Engine</h3><table class="tc-table"><thead><tr><th>Account</th><th>Copy From</th><th>Risk</th><th>Status</th></tr></thead><tbody><tr><td>5205295 • Coinexx</td><td>Wisdo Master</td><td>Auto Governor</td><td><span class="tc-status">Active</span></td></tr><tr><td>5217952 • Coinexx</td><td>Community Reporter</td><td>Equity Ratio</td><td><span class="tc-status">Ready</span></td></tr><tr><td>Mobile Desk</td><td>Selected Account</td><td>Close All Guard</td><td><span class="tc-status">Confirmation</span></td></tr></tbody></table></div></div></div></div><div class="tc-phone"><div class="tc-phone-screen"><span class="tc-kicker">Mobile</span><h3>Performance</h3><div class="tc-gauge"><b>14.8%</b></div><div class="tc-mini"><span>Equity</span><strong>$92,435</strong></div><div class="tc-mini" style="margin-top:10px"><span>Open Trades</span><strong>7</strong></div><div class="tc-spark" style="margin-top:12px"></div><span class="tc-status" style="margin-top:14px">AI Analysis • Bullish</span></div></div></div>`;
}

function tcMarquee() {
  const names = ['MetaTrader 4 • live Reporter','MetaTrader 5 • staged','cTrader • staged','Match Trader • roadmap','TradeLocker • roadmap','DXtrade • roadmap','NinjaTrader • roadmap','Tradovate • roadmap','ProjectX • roadmap','Rithmic • roadmap'];
  const row = names.concat(names).map((n) => `<span>${esc(n)}</span>`).join('');
  return `<section class="tc-marquee"><div class="container"><p class="tc-micro" style="text-align:center;text-transform:uppercase;letter-spacing:.2em">Platform Connector Status</p><div class="tc-track">${row}</div></div></section>`;
}

function tcStatsBar() {
  return `<section class="tc-stats"><div class="tc-stat"><strong>Measured</strong><span>Copy latency shown from telemetry</span></div><div class="tc-stat"><strong>Bounded</strong><span>Idempotent relay architecture</span></div><div class="tc-stat"><strong>Target</strong><span>Uptime objective, not a guarantee</span></div><div class="tc-stat"><strong>Beta</strong><span>Controlled production rollout</span></div></section>`;
}
...
function tcToolsGrid() {
  const tools = [
    ...
    ['📈','Execution Reliability','Telemetry','Runtime health and Reporter receipts expose measured execution state'],
    ...
    ['🔔','Real-Time Alerts','Measured','Notifications report observed delivery state; no zero-delay guarantee'],
...
function accountTradesPage(liveData, membership, state) {
  ...
  const empty = liveData?.live ? '<tr><td colspan="8">Bridge is live, but there are no open or closed trades in the latest reporter snapshot.</td></tr>' : '<tr><td colspan="8">No real MT4/MT5 trade snapshot yet. Pair the Culture Coin Reporter to replace this empty state with live account trades.</td></tr>';
  const replayJson = JSON.stringify(buildPerformanceReplayPayload(liveData, membership, state)).replaceAll('<', '\\u003c');
  return `${renderPairControlGrid(liveData, membership)}
  <section class="wisdo-replay trades-history" data-wisdo-performance-replay>
    <script type="application/json" data-replay-payload>${replayJson}</script>
    <section class="app-panel"><div class="pnl-calendar-head"><button type="button" data-cal-prev>←</button><div><span class="eyebrow">P&L Calendar</span><h3 data-calendar-title>Month</h3><p class="muted">Tap a trading day to open its forensic chart.</p></div><button type="button" data-cal-next>→</button></div><div class="pnl-calendar-grid" data-calendar-grid></div></section>
    <section class="trade-day-drilldown" data-day-drilldown hidden>
      <section class="app-panel"><div class="replay-day-head"><div><span class="eyebrow">Price Structure Replay</span><h2 data-selected-date>Selected trading day</h2><p class="muted">Price structure changes color with direction. Entries and closes are marked on price. Tap a marker for the trade result and stored entry reason.</p></div><div class="replay-pnl positive" data-selected-pnl>$0.00</div></div><div class="replay-chart" data-market-path></div><div class="trade-explanation" data-trade-explanation><strong>Select an entry or close marker.</strong><span>The explanation will show the ticket, direction, entry reason, prices, result and win/loss.</span></div><div class="replay-stats"><div class="replay-stat"><span>Net P&L</span><strong data-stat="net">$0.00</strong></div><div class="replay-stat"><span>Trades</span><strong data-stat="trades">0</strong></div><div class="replay-stat"><span>Win rate</span><strong data-stat="winrate">0%</strong></div><div class="replay-stat"><span>Wins</span><strong data-stat="wins">0</strong></div><div class="replay-stat"><span>Losses</span><strong data-stat="losses">0</strong></div><div class="replay-stat"><span>Lots</span><strong data-stat="lots">0.00</strong></div></div></section>
    </section>
  </section>
  <div class="card" style="margin-top:16px"><div class="terminal-top"><div><span class="eyebrow">Account Trades</span><h3>Live MT4/MT5 trades from the paired reporter</h3><p class="muted">Source: ${esc(liveData.source)} • Last sync: ${esc(liveData.lastSyncAt || 'waiting')}</p></div><span class="tag">${Number(liveData?.metrics?.openTradeCount || 0)} open</span></div><table class="table"><thead><tr><th>Time</th><th>Ticket</th><th>Symbol</th><th>Action</th><th>Status</th><th>Magic</th><th>Price</th><th>P/L</th></tr></thead><tbody>${rows.map((r)=>`<tr><td>${esc(String(r.time || '').replace('T',' ').slice(0,19))}</td><td>${esc(r.ticket || '--')}</td><td><strong>${esc(r.symbol || '--')}</strong></td><td>${esc(r.action || '--')}</td><td>${esc(r.status)}</td><td>${esc(r.magic || '--')}</td><td>${esc(r.price || '--')}</td><td class="metric ${Number(r.pl || 0) >= 0 ? 'green' : 'red'}" style="font-size:18px">${fmtSignedMoney(r.pl)}</td></tr>`).join('') || empty}</tbody></table>${!membership.canCopyTrades ? accessLockNotice(membership) : ''}</div>`;
}
...
function appCalendarLabPage(liveData, membership, state, accountConfig = {}) {
  const replayJson = JSON.stringify(buildPerformanceReplayPayload(liveData, membership, state)).replaceAll('<', '\\u003c');
  const m = liveData?.metrics || {};
  return `<section class="wisdo-os-page wisdo-calendar-lab" data-wisdo-performance-replay>
    <script type="application/json" data-replay-payload>${replayJson}</script>
    <div class="os-commandbar"><div><span class="eyebrow">Trade History • Performance • Price Replay</span><h2>Trading Calendar</h2></div><div class="os-live-chip">● LIVE REPORTER</div></div>
    <div class="os-metric-strip">${appMoneyMetric('Balance', money(m.balance||0))}${appMoneyMetric('Equity', money(m.equity||0))}${appMoneyMetric('Floating', fmtSignedMoney(m.floatingPL||0),Number(m.floatingPL||0)>=0?'green':'red')}${appMoneyMetric('Open Trades', String(Number(m.openTradeCount||0)))}</div>
    <div class="calendar-os-grid">
      <section class="app-panel os-calendar-card"><div class="pnl-calendar-head"><button class="btn" type="button" data-cal-prev>←</button><div><span class="eyebrow">Performance Heat Calendar</span><h3 data-calendar-title>Month</h3><p class="muted">Color = result • brightness = intensity • glyphs = dominant behavior. Tap a trading day to open its forensic replay.</p></div><button class="btn" type="button" data-cal-next>→</button></div><div class="pnl-calendar-grid os-heat-calendar" data-calendar-grid></div></section>
      <aside class="app-panel os-legend"><span class="eyebrow">Heat Meaning</span><h3>Read the scanner</h3><div class="legend-row"><i class="lg profit"></i><span>Favorable / profitable flow</span></div><div class="legend-row"><i class="lg loss"></i><span>Adverse / losing flow</span></div><div class="legend-row"><i class="lg hot"></i><span>Brightness = P&L / activity intensity</span></div><div class="legend-row"><b>↑ ↓ ↕</b><span>Dominant trade direction</span></div><div class="legend-row"><b>C / S</b><span>CORE / SONIC behavior when reported</span></div><p class="muted">The chart below stays hidden until a day is selected.</p></aside>
    </div>
    <section class="trade-day-drilldown" data-day-drilldown hidden>
      <div class="app-panel replay-lab"><div class="replay-day-head"><div><span class="eyebrow">Forensic Price Replay</span><h2 data-selected-date>Selected Day</h2><p class="muted">Price direction, entry, close, result and the Reporter reason live on the same path.</p></div><div class="replay-pnl positive" data-selected-pnl>$0.00</div></div>
      <div class="replay-modebar"><button class="active">FLOW</button><button>RISK</button><button>ENTRY QUALITY</button><button>EXPOSURE</button><button>DRAWDOWN</button><button>STRUCTURE</button></div>
      <div class="replay-chart os-price-heat" data-market-path></div><div class="trade-explanation" data-trade-explanation><strong>Select an ENTRY or CLOSE marker.</strong><span>WISDO will explain what the Reporter stored for that decision.</span></div>
      <div class="replay-stats"><div class="replay-stat"><span>Net P&L</span><strong data-stat="net">$0.00</strong></div><div class="replay-stat"><span>Trades</span><strong data-stat="trades">0</strong></div><div class="replay-stat"><span>Win rate</span><strong data-stat="winrate">0%</strong></div><div class="replay-stat"><span>Wins</span><strong data-stat="wins">0</strong></div><div class="replay-stat"><span>Losses</span><strong data-stat="losses">0</strong></div><div class="replay-stat"><span>Lots</span><strong data-stat="lots">0.00</strong></div></div></div>
    </section>
  </section>`;
}

function appServicesCockpitPage(liveData, membership, state) {
  const m=liveData?.metrics||{}, live=Boolean(liveData?.live);
  const nodes=[
    ['MT4 Reporter',live?'LIVE':'WAITING',liveData?.lastSyncAt||'No sync yet'],
    ['WISDO Kernel','READY','Intent → permission → execution'],
    ['Copier Engine',membership.canCopyTrades?'READY':'LOCKED',membership.canCopyTrades?'Execution enabled':'Membership gate active'],
    ['Discord','CONNECTED',membership.linkedDiscordUserId?'Identity linked':'Connection available'],
    ['Voice / Pi','AVAILABLE','Natural-language command surface'],
    ['Automation','READY','Rules, schedules and protection']
  ];
  return `<section class="wisdo-os-page services-cockpit"><div class="os-commandbar"><div><span class="eyebrow">WISDO Operating Services</span><h2>Services Cockpit</h2><p class="muted">What WISDO is connected to, what is healthy, and what is acting right now.</p></div><div class="os-live-chip">${live?'● SYSTEM LIVE':'○ WAITING FOR REPORTER'}</div></div>
  <div class="os-metric-strip">${appMoneyMetric('Balance',money(m.balance||0))}${appMoneyMetric('Equity',money(m.equity||0))}${appMoneyMetric('Open Trades',String(Number(m.openTradeCount||0)))}${appMoneyMetric('Reporter',live?'Online':'Waiting',live?'green':'gold')}</div>
  <div class="service-tabs"><span class="active">Overview</span><span>Trading</span><span>Copier</span><span>Voice</span><span>Discord</span><span>Automation</span><span>Devices</span><span>Health</span></div>
  <div class="services-grid"><section class="app-panel service-map"><span class="eyebrow">Live System Map</span><h3>Connected WISDO services</h3><div class="service-node-grid">${nodes.map(([n,status,detail],i)=>`<article class="service-node ${status==='LIVE'||status==='READY'||status==='CONNECTED'?'online':status==='LOCKED'?'locked':''}"><span class="node-pulse"></span><small>SERVICE 0${i+1}</small><h3>${esc(n)}</h3><strong>${esc(status)}</strong><p>${esc(detail)}</p></article>`).join('')}</div></section>
  <aside class="app-panel activity-rail"><span class="eyebrow">WISDO Activity</span><h3>System pulse</h3><div class="activity-event"><i></i><div><strong>Reporter</strong><span>${esc(liveData?.lastSyncAt||'Waiting for first sync')}</span></div></div><div class="activity-event"><i></i><div><strong>Account</strong><span>${esc(liveData?.accountId||'No selected account')}</span></div></div><div class="activity-event"><i></i><div><strong>Trade state</strong><span>${Number(m.openTradeCount||0)} open • ${fmtSignedMoney(m.floatingPL||0)} floating</span></div></div><div class="activity-event"><i></i><div><strong>Command security</strong><span>Permission + confirmation gates active</span></div></div></aside></div></section>`;
}
...
function portalContent(page, membership, state, selectedAccountId = '') {
  ...
  if (page === 'calendar') return appCalendarLabPage(liveData, membership, state, accountConfig);
  if (page === 'services') return appServicesCockpitPage(liveData, membership, state);
  ...
  if (page === 'account-trades') return `<section class="app-panel"><div class="terminal-top"><div><span class="eyebrow">Trade Ledger</span><h3>Individual MT4/MT5 trades</h3><p class="muted">The calendar, trading-day replay and flow heat map above are synchronized to this account. The live trade ledger stays below for ticket-level inspection.</p></div></div>${accountTradesPage(liveData, membership, state)}</section>`;
  if (page === 'performance') return appPerformanceProductPage(liveData, membership, state, accountConfig);
